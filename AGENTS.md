...
- `src/util/git-auto-commit.ts` — unattended ULW **Cycle complete.** creates a local git commit (never push); `FORGE_ULW_AUTO_COMMIT=0` off
...
- `src/harness/ulw-cycle.ts` — ULW cycle flag 1/0; optional `maxWaves` cap (auto LAST on Stop; **capped idle epochs do not increment `w`** — the cap is work units, not ~20 tool rounds; unlimited still stamps idle epochs); general/evaluate-class mandates keep their verbs (written reading is Wave 1, not advice); follow-up user text does not re-arm or replace the mandate (`/ulw <new>` still does); soft prompts expand to god-scope; Stop blocks while cycle=1; wave ledger + quality bar (proof demands, thin-wave escalation, evidence attestation); net-diff progress tracking (`gitDiffFingerprint`: bash-channel edits count as progress, edit→revert churn = revisit → thin + excluded from bestWave); background bash excluded from structural verification (`countsTowardVerification` — spawn observes no exit code)
