...
- `src/agent/` — loop, tools, permissions, subagents (`spawn_subagent`; general-purpose defaults to `isolation=worktree` and auto-lands into parent only when status=completed; incomplete keeps the worktree). Credential reads (`auth.json`, `id_rsa`) are hard-denied — YOLO / `--read-outside allow` cannot dump tokens into the model.
