...
- `src/tui/bottom-status.ts` — always-on REPL bottom dock (model · ctx · `cache N%` last-round · plan quota · reset · `sub N $x` when children spent); `FORGE_BOTTOM_STATUS=0` off
