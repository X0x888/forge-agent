...
2. Every driver must be able to release: `/goal` stuck-wall (when ULW is off), ULW `Verdict: fulfilled` / `/cycle 0` / `max_cycles` / `/ulw-off`, no-progress wall without a mandate, guard caps. Never an infinite trap without progress.
