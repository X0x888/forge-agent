...
- `src/harness/` — the product:
  - `stop-guard.ts` composes, in order: user Stop hooks → `report-guard.ts` (attestation pass for `**Goal achieved.**`, **before the drivers**) → `goal.ts` (skipped while ULW is armed) → `cycle/` (the ULW driver) → `todo-gate.ts` → `handoff-guard.ts` → `proof-claim-guard.ts` → `report-guard.ts`. Every block is counted per guard in `guardBlocks` (run JSON · `metrics.jsonl` · `forge stats` harness row).
  - `cycle/` — the ULW plan-cycle driver: `state.ts` (schema-2 `ulw.json`: cycle, phase, plan items, `cycles[]`, ledger), `machine.ts` (pure `decideAtStop`), `artifacts.ts` (`scout.md` / `plan.md` / `look.md` / `review.md` parsers — labelled lines, never intent; a `continue` plan owes nonempty `Looked:` / `Direction:` / `Worth the cycle:`, `Considered:` with `leave it`, `serves:` and `red now:` per item, at most 12 items (a backlog, not a session), `One item:` when there is only one), `briefs.ts` (what the fresh Planner / Reviewer are handed — in two turns: the product first, the record second — and what the executor hears of the last review at the next plan admission), `roles.ts` (the two-turn runner on one kept subagent session; `FORGE_ULW_TWO_TURN=0` = one brief), `orchestrator.ts` (runs the roles, the harness-run verify and the commit through an injected `CycleRuntime`; `ensureCyclePlanned` at turn start), `verify.ts`, `controls.ts` (`/cycle 0` = finish the open cycle then stop; `/replan`; `/max-cycles`), `status.ts`, `peer-scout.ts` (background 1–3 GitHub peers of this Identity; staged `peers.md` for the next Planner after Looked:; executor never sees it; `FORGE_ULW_PEERS=0` off). There is **no mandate classifier and no wave meter** (the user's words are never rewritten — they are attention, not a quality ceiling; `Direction:` is the Planner's after using the product): the Planner decides `fulfilled`, the Reviewer judges the diff, the harness enforces sequence and facts. On a **no-mandate** run the model cannot stop itself — a scout that already parses as a `continue` plan is admitted; otherwise a plan-less Planner or a `fulfilled` becomes a synthesized work cycle (keep-promise / go-deeper / direct-execute), and the run releases only on a mandate fulfilled after sitting the product, a user control, or — with no mandate — the no-progress wall on repeating empty work (`directExecuteStreak` / `noCommitStreak` ≥ `FORGE_ULW_NO_PROGRESS_CAP`; an open mandate, a Planner `blocked`, or a dead look does not end the mill); a read-only role (Planner `denyEdits`) never hears the fix-until-green nudge (`toolSetCanEdit`), and its plan turn is `documentOnly`. Facts still gate everything (no writes before a plan, no executor `git commit` while armed, no commit before a completed review and a green harness-run check — green is "no new failures against the baseline", never an isolate; a `cargo -p && godot --headless` compound is the product gate) — the baseline is captured the first time each gate command is seen: cycle 1's tree, or the tree as the last commit left it when a later Planner declares a different check, then the accepted run after each commit; the order is verify → review → verify → commit so the Reviewer reads a tree that passes). `verification.ts` / `verify-command.ts` / `declared-checks.ts` classify what a check run is.
...
- `src/mcp/`, `src/lsp/` — MCP (defaults context7 + playwright on a session look profile, or `--isolated`) and LSP ensure packs. GitHub source is the native `github` tool. `look_native` / `look-server.ts` / `look-chrome.ts` / `look-script-refuse.ts` are the native/Vite look path (dead Playwright reconnects on `look-N`; harness Chrome sits the look URL).
...
2. The mill keeps working — that is the design, not a bug. A long, expensive run that delivered little is a **work** defect (plan / look / worth / verify); improve those so the hours pay, do not add a cap or stop the mill earlier. Drivers still release: `/goal` stuck-wall (when ULW is off), mandate `fulfilled` / `/cycle 0` / `max_cycles` / `/ulw-off`, no-progress wall without a mandate, guard caps. Never an infinite trap *without progress*.
...
- Real ULW runs are the ground truth for harness changes: `~/.forge/sessions/*/ulw.json` (`cycles[]`, ledger) and `cycles/<n>/plan.md` / `review.md`. Survey them before adding a rule. A long mill that delivered little is a work defect — improve the work, do not add a cap.
