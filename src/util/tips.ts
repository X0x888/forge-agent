...
export function expertTipsLines(): string[] {
  return [
    `Forge expert tips`,
    `  Live mid-run:  /improve  ·  /hooks init  ·  /cycle 0|1  ·  /max-waves N|off  ·  /ulw-off  ·  /budget N|off  ·  /notify on  ·  /plan  ·  /build  ·  exit_plan_mode  ·  /model  ·  /pause  ·  /unpause  ·  /done  ·  /status  ·  /checkpoint  ·  /tasks kill|log  ·  get_task_output wait=  ·  !cmd  ·  @path`,
    ...
    `  Project stack:  auto pm+checks+monorepo in prompt · /context · doctor/status/run JSON · /memory project · memory_write scope=project · /init seeds memory · FORGE_VERIFY_HINT=0 · FORGE_FILE_READ_GUARD=0 · FORGE_AUTO_VERIFY_NUDGE=0 · FORGE_FIX_UNTIL_GREEN=0`,
    ...
    `  Safety:        /plan → design (session-only)  ·  Context7 query-docs is plan-safe  ·  exit_plan_mode or /build → implement  ·  /permissions acceptEdits|dontAsk (sticky)  ·  --sandbox workspace  ·  --sandbox-missing fail-closed  ·  --read-outside deny  ·  /diff (argv-safe)  ·  deny Bash not Bash()  ·  no curl IMDS 169.254.169.254  ·  no curl file://`,
    `  Accounts:      forge accounts list|switch|auto-switch  ·  forge login --add  ·  /accounts  ·  /auth  ·  auto-switch on 429/quota + plan threshold`,
    `  Attention:     /bell on  ·  /notify on  ·  /budget N  ·  /copy  ·  /last  ·  /files  ·  /path  ·  /logs  ·  /config  ·  /pin  ·  /stats 7|week  ·  /news all  ·  /share  ·  /memory  ·  /checkpoint  ·  /retry  ·  unknown /cmd → Did you mean?`,
    ...
    `  Dirty trees:    doctor warns at ≥20 changed files under ULW · ULW arm auto-checkpoints (FORGE_ULW_CHECKPOINT=0 off) · /checkpoint restore`,
