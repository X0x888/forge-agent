...
export function expertTipsLines(): string[] {
  return [
    ...
    `  Accounts:      forge login --add  ·  forge login -p cursor --oauth --add  ·  forge login -p copilot --add  ·  forge accounts  ·  forge accounts switch <email>  ·  /accounts  ·  /auth  ·  same email without --add updates in place  ·  auto-switch on 429/quota + plan threshold  ·  /fallback after 429/5xx (off by default; floor grok-4.5 high — never a weaker model)`,
    ...
    `  Recovery:      /undo  ·  /checkpoint (/snap)  ·  /retry  ·  /sessions errors  ·  /context  ·  /compact  ·  sessions prune --orphans  ·  forge tmp prune  ·  /memory project prune  ·  /init  ·  forge logs`,
    ...
    `  Session inventory: sessions list --json · doctor Recommended · ≥100 sessions · leftover ~/.forge/tmp · orphan subagents · forge sessions prune --keep 50 · --orphans · forge tmp prune`,
