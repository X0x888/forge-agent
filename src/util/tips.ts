...
export function expertTipsLines(): string[] {
  return [
    `Forge expert tips`,
    `  Live mid-run:  /cycle 0|1  ·  /ulw-off  ·  /pause  ·  /unpause  ·  /done  ·  /status  ·  /tasks kill|log`,
    ...
    `  CI:            forge run "…" --title job --json  ·  forge "…" --json  ·  forge run "…" --continue (fail-closed)  ·  never --continue --new · never --session --continue  ·  session_not_found/continue_miss → suggestions[]  ·  empty run → ok:false reason=empty_run  ·  run --json sessionPath+forgeHome  ·  forge models -p xai --json  ·  -p claude|gpt|oai|copilot  ·  FORGE_PROVIDER=claude  ·  forge login --from-copilot  ·  --sandbox readonly  ·  forge auth --json  ·  forge login --api-key $KEY --json  ·  login -p sticks provider  ·  FORGE_JSON_COMPACT=1  ·  FORGE_MAX_RUN_MS=30m  ·  forge doctor --json  ·  doctor/config what-if --sandbox/--read-outside  ·  run --json productionWarnings[]  ·  --max-turns N  ·  unknown flags + --json → unknown_option  ·  bare typos/aliases (cfg→config, whoami→auth) → Did you mean?`,
    `  Safety:        /permissions plan|acceptEdits|dontAsk  ·  --sandbox workspace  ·  --sandbox-missing fail-closed  ·  --read-outside deny  ·  /diff (argv-safe)  ·  deny Bash not Bash()  ·  no curl IMDS 169.254.169.254  ·  no curl file://`,
