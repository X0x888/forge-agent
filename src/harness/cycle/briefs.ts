...
function recordLines(input: PlannerPlanInput): string[] {
  ...
  if (s.cycles.length) {
    ...
    lines.push(
      ...
      `A cycle is one executor session of a theme. Pack the class you just looked at. A ledger row that shows \`1 item\` is the exception — One item: a real isolation, not another slice of the same job.`,
    );
...
export function buildPlannerPlanBrief(input: PlannerPlanInput): string {
  ...
  const lines: string[] = [
    ...
    `Your scout is below, then the record of this run. Strike what is done, find the class if there is one, pack that class into one session-sized plan (one item is the exception and needs One item:), price the session against leave it, write the plan. You do not implement.`,
    ...lookPathLines({
      lookProfileUdd: "lookProfileUdd" in input ? input.lookProfileUdd : undefined,
      workspace: input.workspace,
      sessionId: input.state.sessionId,
...
const REVIEWER_DUTY = [
  ...
  `- Class is this cycle's scope. Considered candidates of the same Looked: job that are not in Items: are unfinished work in this cycle, not Architecture for cycle N+1. A one-slice plan of that class is Worth: no — the harness packs those siblings onto the board. Do not invent a different product.`,
  `- Must-fix names what a user of this product would hit if it shipped. A finding whose repair changes nothing they get is taste, not Must-fix. Specificity is the floor: the thing, the evidence, the next action.`,
  ...
  `- Do not start the next cycle's work.`,
];
...
function planSessionFacts(planText: string): string[] {
  ...
  if (n === 1 && considered >= 2) {
    ...
    if (plan.outOfScope.length) {
      bits.push(
        "Out of scope on a one-item plan is one different job or a lease limit, not the rest of this class.",
      );
    }
    return bits;
  }
