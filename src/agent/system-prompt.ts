...
export function buildBaselineSystemPrompt(opts: {
  ...
  if (ulwOn && (opts.subagentDepth ?? 0) === 0) {
    parts.push(
      ...
      `- Tests must catch a plausible fault; never weaken an assertion to go green. Test-only work can add meaningful regression protection or resolve a concrete evidence gap. Do not require production edits when behavior is already correct.`,
