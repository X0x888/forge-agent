...
export function buildBaselineSystemPrompt(opts: {
  ...
  const parts: string[] = [
    ...
    `- Ambiguous / multi-option / architectural work: call enter_plan_mode before implementing — do not wait for the user to type /plan. Then research and exit_plan_mode.`,
    ...
    `- **/ulw god-mode**: cycle=1 forces research→judge→implement→prove→review on the highest-leverage hard work; cycle=0 = wrap in-flight work + already-named ships, then **Cycle complete.** Durable decisions live in decisions.json.`,
  ...
  if (ulwOn && (opts.subagentDepth ?? 0) === 0) {
    if (resolveUlwPhase(opts.ulwCycle) === "orient") {
      ...
      parts.push(
        ...
        `Smoke/orient → judge leverage → research only as needed (MCP/LSP/subagents) → ship one objective (defect-class siblings only) → prove → serendipity if cheap → hostile review → repeat while cycle=1; if cycle=0 wrap in-flight work + named plan (or cancel with reason), then attest **Cycle complete.** with evidence.`,
        ``,
