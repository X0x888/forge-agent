  ...
  if (ulwOn && (opts.subagentDepth ?? 0) === 0) {
    if (resolveUlwPhase(opts.ulwCycle) === "orient") {
      ...
      parts.push(
        ...
        ``,
        `### Subagents (proactive)`,
        `Spawn explore/plan/general-purpose **whenever** that improves quality or efficiency (parallel map, clean design space, isolated implement, worktree isolation). Skip when one call is enough. The child result is the brief — follow \`- Next:\` on the handoff (read artifact; resume_session_id for incomplete implementers; do not re-explore unless pick: is missing). skipped_explore_ledger is not a look. Converge and ship in the parent.`,
        ``,
        `### Wave loop`,
        ...
        `Smoke, prove, hostile review. Repeat while cycle=1. \`/cycle 0\`: finish this wave, one more, LAST. LAST reflect then **Cycle complete.** Imagine + read_file screenshots when the product must look like something.`,
        ``,
