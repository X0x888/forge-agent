...
- **Session `lastError`**: provider/run failures stamp `meta.lastError` with code/message/tips; `/status`, resume orientation, `/share`, `sessions show`, and `forge status --json` surface recovery (`lastError` + `ERR:<code>` tag); cleared on the next successful turn, `/clear`, and `/fork`. `/config` tips plan → `/build`
