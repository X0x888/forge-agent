...
- **Look/compile junk is not a product edit** (`src/agent/tools/bash-mutation-journal.ts`, `src/util/git-ensure.ts`): pixel-pets journaled 13k `.swiftmodule` rows under `.forge/tmp-c3-look`. Those paths (and `*.o`) skip the journal; the starter `.gitignore` lists `.forge/tmp-*/` and `*.swiftmodule`. Job: a HostCareCheck compile does not bump `editCount`.
