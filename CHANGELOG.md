...
### Added
- **Unlimited ULW keeps the job at wave 125**: duration stays uncapped — there is no hidden `max_waves`. Compact, CONTINUE re-anchor, and decision-memory now reprint a **job card** (verbatim Wave-1 `Reading:`, open named ships, last job-moving ship, last play/look) so mill suffix cannot lobotomize the thesis. `bestWave` crowns job movement (named/pick/play/full-suite/control-flow), not the fattest sibling mill. Three numbered `foo-n.js` / same-dir new-module ships hold and demand explore/play + a new Reading (maze regex stays fixtures). Three off-job credited ships re-arm PLAN; explore skip yields when the last ships missed the job. Thought-only streaks in a cycle force a look, not another silent judge (still no auto-LAST). Consolidation runs the suite in the background and waits. `forge-veteran` inlines under ULW. Family budget reserves a look slice for explore on re-PLAN. Auto-commit skips mill-only trees so a clean commit cannot launder mill into `net=new`. Job: long uncapped `/ulw` still knows Wave 1 after compact; mill factories do not become the bar.
