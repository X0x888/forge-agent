...
### Fixed
- **Dead Playwright is down the moment the child exits** (`src/mcp/client.ts`, `manager.ts`, `src/util/jsonrpc-stdio.ts`, `src/harness/cycle/orchestrator.ts`): last night `tools/list` then `mcp:playwright exited (code=0)` still left screenshot in the registry and the scout brief on `connecting`. Exit drops tools immediately; idle after death is `down` (look_native), not `connecting`; a qualified `call_mcp` still reconnects. Plan admission no longer spawns a second harness Chromium — four mills already died from concurrent Chrome. Job: `search_mcp` after exit 0 does not still list a screenshot.
- **Never-sit-down sits the product** (`src/mcp/client.ts`, `defaults.ts`, `manager.ts`, `src/agent/browser-lease.ts`, `src/util/look-chrome.ts`, `look-server.ts`, `src/agent/tools/look-native.ts`, `src/harness/cycle/orchestrator.ts`, `src/util/git-auto-commit.ts`): a dead Playwright child is not `ready` (tools drop; briefs say down); reconnect rotates `browsers/look-N` and kills leftover Chrome on the old UDD; when MCP is down the harness screenshots its own Chromium of the look server (`look_native`, never Chrome.app, never the TUI); Chrome extensions serve `public/` not leftover `dist/`; an unpaid skip restores the tree then plans the next cycle. Empty-work walls still do not release an open mandate. Job: overnight `/ulw` keeps going on a living sit and a clean tree, not Ghostty and 20 leftover files.
- **The mill does not sit down with the job unfinished** (`src/harness/stop-guard.ts`, `cycle/orchestrator.ts`, `src/agent/loop.ts`, `reasoned-stop.ts`, `src/config/preferences.ts`): thought-only under ULW/`/goal` keeps forcing a tool instead of returning the TUI to `›`; an armed ULW skips the `/goal` stuck-wall so three no-edit Stops cannot swallow `Plan complete.` or kill the mandate; empty-work walls (`noCommitStreak` / `synthStreak`) do not release while a mandate is still open; a dead look does not burn `synthStreak`; mandate `fulfilled` requires sitting the product, not a README `Looked:`; leftover `/setup 2` $5 is not a mill fuse (`budgetExplicit`). Job: overnight `/ulw` keeps going until the mandate is met, `/cycle 0`, or — with no mandate — repeating empty work.
- **Playwright MCP reconnects after the child exits 0** (`src/mcp/client.ts`, `defaults.ts`): overnight mills listed tools then every `call_mcp` was `mcp:playwright exited (code=0)`. Ready state did not notice a dead stdio child. `ensureReady` / `callTool` reconnect once. `--isolated` plus `--user-data-dir` (Playwright throws, still exit 0) is stripped. User `mcp.json` `@latest` is pinned to the same spec as the built-in. Each session look profile gets its own `--output-dir`. Job: four concurrent `/ulw` mills `call_mcp` a screenshot of `127.0.0.1:<lookPort>`.
- **Planner blocked and exhausted fix rounds do not kill the mill** (`src/harness/cycle/orchestrator.ts`, `index.ts`): Hearth released on the first Playwright-down `Verdict: blocked`; HashPet released on `fix-cap` of a 16px sit it could not look. A blocked Planner synthesizes other work. Fix-round exhaustion skips the cycle and plans the next. A dead look does not increment `noCommitStreak`. A plan `Looked:` that claims a live sit the scout said never ran is not admitted. Empty synthesized cycles still cap. Job: a dead look lease is a class to leave, not `/ulw` ending.
- **`Bash(rm -rf /)` does not deny `/tmp/...`** (`src/agent/rules.ts`): the deny regex was a prefix, so `rm -rf /tmp/hashpet-c11-cft` (CFT relaunch) matched `rm -rf /`. Patterns without `*` are whole-segment. Job: a mill that falls back to Chrome for Testing under `/tmp` is not hard-denied.
- **Unlimited ULW is not nagged to set a spend cap** (`src/util/production-warnings.ts`, `setup-readiness.ts`, `hints.ts`): doctor/setup/`/status` treated missing `/budget 5` as a mill fuse. Unlimited is the product; `/budget` stays an optional human brake. Job: overnight `/ulw` is not told to stop at $5.
- **Playwright MCP never borrows the user's Chrome.app on macOS** (`src/mcp/playwright-browser.ts`, `src/mcp/defaults.ts`): `@playwright/mcp` defaults to the `chrome` channel, and a headless copy of /Applications/Google Chrome.app registers with LaunchServices as `com.google.Chrome` (BackgroundOnly), so opening Chrome from the Dock activated the windowless look browser and Chrome "hung" with no window whenever a session looked. `decoratePlaywrightServer` now appends `--executable-path` to the newest complete Playwright Chrome for Testing / Chromium (then chrome-headless-shell for a headless look), or `--browser chromium` when none is installed. An entry that already picks a browser (`--browser`, `--executable-path`, `--cdp-endpoint`, `--extension`, `--config`, `PLAYWRIGHT_MCP_*`) and other platforms are untouched; `FORGE_PLAYWRIGHT_MANAGED_BROWSER=0` restores the channel. Job: Chrome opens from the Dock while three mills look.
- **Playwright MCP uses the session look profile** (`src/mcp/defaults.ts`, `manager.ts`, `src/util/jsonrpc-stdio.ts`, `src/mcp/client.ts`): with a session id, `--isolated` is replaced by `--user-data-dir ~/.forge/sessions/<id>/browsers/look`. Default args include `--headless` (`FORGE_PLAYWRIGHT_HEADLESS=0` for a window). Stdio JSON-RPC writes a trailing newline so Playwright's NDJSON SDK completes `initialize` (Content-Length alone hung forever). Ready is tools/list — Playwright has no resources/prompts. Job: a Vite mill's scout `call_mcp`s a screenshot.
- **Harness starts the look server on the session port** (`src/util/look-server.ts`, `src/harness/cycle/orchestrator.ts`, `briefs.ts`): Vite (or `npm run dev`/`preview`) binds `lookPortForSession` with `--strictPort`, registers a GUI lease, and the brief names `http://127.0.0.1:<port>`. `FORGE_LOOK_SERVER=0` off. Job: Scrapfall does not drive 魔塔's :5173.
- **Look/dev-server lease, native HID sit, docs-only review, operator-file gate** (`src/agent/tools/bash.ts`, `browser-lease.ts`, `look-native.ts`, `src/harness/cycle/orchestrator.ts`, `verify.ts`, `src/harness/project-memory.ts`): `npm run dev` / `vite preview` pin the session look port, background, and reuse a live lease — a model `timeout_ms: 30000` no longer SIGTERM's the look (`FORGE_LOOK_DEV_LEASE=0` off). `look_native` click / simctl io / Godot `--write-movie` with TCC classify-on-fail. A docs-only cycle skips the 25-turn Reviewer look. ULW does not rewrite tracked `.forge/MEMORY.md` (JSON store stays). Named verify failures that cite `.forge/MEMORY.md` or `AGENTS.md` are not product-gate red. Job: the next mill does not kill Vite at 30s, spend 25+80 looking at a MEMORY restamp, or fix-round an operator-file grep.
- **`looks/*.mjs` Chrome/CDP scripts are refused** (`src/agent/tools/look-script-refuse.ts`): `write_file` / `search_replace` / `apply_patch` of `looks/*.mjs` and chrome/CDP look scripts error and tell the model to `call_mcp playwright`. Job: a down Playwright does not mill a CDP launcher.
- **Vision 400 no longer kills an unattended mill** (`src/util/user-images.ts`, `src/util/raster.ts`, `src/providers/errors.ts`, `src/agent/loop.ts`): QQHX died on `read_file` of a dump `.bmp` (`invalid_image` — not JPG/PNG/WebP/ICO); QQT's Planner died on a 15×17 sprite (255 pixels < xAI's 512). BMP converts to PNG; GIF is not attached; edges below 8px stay dropped; icons with both edges ≥8 but area <512 upscale. Any remaining vision 400 strips `image_url` and retries once instead of `lastError` stopping ULW. Job: a dump BMP and a 15×17 PNG do not end `/ulw`.
- **Playwright MCP actually looks** (`src/mcp/defaults.ts`, `manager.ts`, `tools.ts`, `src/util/look-port.ts`, `src/harness/cycle/briefs.ts`, `src/util/product-kind.ts`, `src/util/jsonrpc-stdio.ts`): `@playwright/mcp` is pinned (not `@latest`); `search_mcp` waits while Playwright is connecting; a configured-but-down server is `isError` and says not to write CDP `looks/*.mjs`. A dead stdio child's EPIPE is fail-closed, not an uncaught crash. Each session gets its own look port so Scrapfall cannot drive 魔塔's Vite. Swift/AppKit, macroquad, and `.xcodeproj` trees skip Playwright (Godot already did). Job: a Vite mill's scout `call_mcp`s a screenshot; a QQHX mill does not prewarm Chrome.
- **Native look is limited, not a 25-turn osascript mill** (`src/util/look-infra.ts`, `src/agent/tools/look-native.ts`, `/doctor`): TCC -10004 / simctl LS 115 / EPERM classify as `limited` once. Unclassified `screencapture` failures stay errors, not a fake TCC. `/doctor` recommends granting Accessibility when the probe is denied. Job: System Events -10004 does not refill keep-promise.
- **Look kind is a receipt, not `load`/`watch` prose** (`src/harness/cycle/artifacts.ts`, `src/agent/tools/look-native.ts`, `src/util/look-infra.ts`): AUTO stills and `cargo run -- --help` cannot satisfy a surface sit; leased Chrome is `cdp-live` even when Playwright is down. Mentioning `look_native` or `call_mcp` after a failed sit is not a HID/CDP receipt. Look/scout last turn keeps tools (QQHX c1 last-turn lock). Job: Chain Bench cannot ship first-hour from `LOAD A CRATE. WATCH THE CHIPS.`
- **Look-only red verify is not a fix-cap** (`src/harness/cycle/orchestrator.ts`, `src/util/git-auto-commit.ts`): Pixel Pets cycle 15 shipped a look with no product diff, then `swiftc HostCareCheck` went red and released the run. A red gate on a tree with no substance diff skips the cycle instead of burning `fix_rounds`. `.pcm` / `looks/*.mjs` / `look-shots/` / `.forge/MEMORY.md` are not product edits. Job: a SimulatorKit HID look does not fix-cap.
- **Substance synths do not burn `FORGE_ULW_SYNTH_CAP`** (`src/harness/cycle/orchestrator.ts`, `status.ts`, `docs/ULW.md`): empty synthesized cycles still cap; cycles that land substance do not (QQT c40–41 / SCRAPFALL c12–13). `/report` names synthesized cycles and the last SHA instead of `0 cycle(s) landed nothing`. Job: a mill that actually shipped does not suicide on the synth cap.
- **`Worth: no` does not commit** (`src/harness/cycle/orchestrator.ts`, `artifacts.ts`): a Reviewer `Worth: no` skips the substance commit. `.forge/MEMORY.md` is not a cycle commit or an architecture class; a prior `Collapse \`ident\`` title retires must-collapse. Scout-as-plan salvages a parseable scout when turn 2 400s after cycle 1. `FAIL ident:` is a named verify failure. Keep-promise still packs up to five unkept promises (cycle is a session). Job: the next overnight `/ulw` does not commit a Worth: no collapse.
- **ULW quota failover ignores auto-switch off** (`src/auth/accounts.ts`, `src/agent/loop.ts`): Cursor `resource_exhausted` with auto-switch disabled killed every leftover Cursor role at once. An armed ULW still tries another same-provider account. Job: a mill does not die because `/accounts auto-switch` was off.
- **Cursor `agent-transcripts` 404 is one hint** (`src/agent/tools/path-hints.ts`): glob of `~/.forge/cursor-projects/.../agent-transcripts` is not Cursor. Job: the model stops listing a path Forge never creates.
- **rustfmt reject is loud** (`src/agent/tools/format-on-write.ts`): Yardfowl `combat.rs` kept going after `unexpected closing delimiter`. Job: the next turn fixes parse, not another feature.
- **CLI setup Next writes the spend cap** (`src/util/setup-readiness.ts`): `forge setup` printed `forge setup budget` after omit-N became peek, so typing the Next key reprinted the card and left the cap unlimited; `/setup 2` peeks instead of writing $5. Next / `--json` `action` is `forge setup budget 5`; numbered `/setup 2` still writes $5; bare `forge setup budget` still peeks. Job: type the setup Next key, `forge config --json` shows a finite cap.
- **`forge help` includes the Examples footer** (`src/cli/help-groups.ts`): `forge help` used `helpInformation()` so grouped Usage printed without the `addHelpText("after")` Examples/Docs block `--help` already had. Same footer on both; `help --json` `help` string includes Examples. Job: type `forge help` unsigned, see Examples.
- **REPL empty pinned does not teach `/pin <id>`** (`src/cli/sessions-recovery.ts`): unifying `formatPinnedEmpty` printed `/pin <id>`, which `/pin` rejects (`Unknown /pin arg`). `/pin` protects this session; `/sessions pin <id>` protects another. CLI still `forge sessions pin <id>`. Job: type `/sessions pinned` with none, the Next key exists.
- **`sessions delete --dry` does not delete** (`src/cli.ts`): `--dry` is a sessions-command flag, so `forge sessions delete <id> --dry` ignored it and unlinked. Non-prune `--dry` is usage; JSON `doctor --dry` no longer carries Did-you-mean `--deny`. Job: type `forge sessions delete <id> --dry`, the dir stays.
- **Longer English is not a command typo** (`src/cli/suggest-command.ts`): `helper` / `runner` still scored as `help` / `run` via the length-6–9 edit cap. Extra letters beyond one stay a run; `helpp` still recovers. Job: `forge "helper" --json` is the same fail-closed run as `forge run "helper" --json`.
- **CLI `forge setup 1` is not model confirm** (`src/util/setup-readiness.ts`): `parseSetupAction` mapped leftover digits 1–6 to verbs, so isolated `forge setup 1` printed “Provider/model confirmed” and set `seenProviderModelConfirm` while `setup --help` is verbs-only. CLI digits are usage; `forge setup model` still persists; REPL `/setup 1` still confirms. Job: type `forge setup 1`, preferences stay unconfirmed.
- **`--orphans` is mills, not keep-50** (`src/session/session.ts`, `src/cli.ts`): `forge sessions prune --orphans --dry` still applied omitted `--keep` as 50 (`overKeep || orphan`), so one mill preview listed forty-seven real sessions. Omitted `--keep` with `--orphans` is mills only; `--keep 50 --dry` is still the session-count cull; explicit `--keep 50 --orphans` ORs both. `--dry-run` aliases `--dry`. Doctor session-count Next is `forge sessions prune --keep 50 --dry`. Job: type `--orphans --dry`, would-drop equals mill count.
- **Sessions prune `--dry` previews keep and orphans** (`src/session/session.ts`, `src/cli.ts`): `--keep 50 --dry` and `--orphans --dry` were usage (`forge sessions prune --journals --dry`) while `--help` already advertised prune preview. `--dry` now lists would-drop and leaves dirs; a following prune without `--dry` deletes. Journals `--dry` still only drops `mutations.jsonl`. Job: type doctor session-count Next, add `--dry`, see the would-drop.
- **Hygiene `--dry` is not `--deny`** (`src/cli.ts`, `src/session/mutations.ts`): `forge sessions prune --journals --dry` was unknown and Did-you-mean `--deny` (same on `doctor --dry` / `stats --dry`); `tmp prune --dry` already previews. Journal prune now previews and leaves `mutations.jsonl`; a following prune without `--dry` deletes. Unknown `--dry` points at tmp/journal preview, not permission `--deny`. Job: type doctor journal Next, add `--dry`, see the would-drop.
- **Session recovery verbs live in one module** (`src/cli/sessions-recovery.ts`): leftover mixed copy (`slash.ts` doctor bodies, `forge sessions pinned` empty, `--title` `list -q / /sessions search`, timeout tips, orphan rec) forked CLI vs REPL. `slash.ts` imports the table rather than owning it. CLI `forge doctor` / `sessions --help` print `forge sessions errors` / `untitled` / `pin` from that module. Job: after `forge doctor` and `forge sessions --help`, every recovery key comes from one owner.
- **Doctor recovery is one table** (`src/tui/doctor-card.ts`): CLI vs REPL recovery keys (`errors` / `untitled` / `pinned` / `pin` / `unpin`) live in `doctorSessionsRecoveryVerb`. `forge doctor` pinned lines and `forge sessions --help` `--force-last-error` use the CLI verbs, not `/sessions errors`. REPL still prints `/sessions errors`. Job: after `forge doctor` and `forge sessions --help`, every recovery key is a shell command.
- **Doctor closer keeps the rebuild; CLI body is shell** (`src/tui/doctor-card.ts`): a crowded `forge doctor` Next (`login` · yolo · journals · keep-50) dropped `bash install.sh`, and the dump still said `/sessions errors` / `/sessions untitled`. The rebuild stays in the four Next keys; CLI lastError/untitled lines are `forge sessions errors` / `forge sessions untitled`. REPL may keep slashes. Job: type Next after `forge doctor`, including `bash install.sh` when dist lags.
- **`listSessions` lists directories, not titles** (`src/session/session.ts`): leftover Chrome look dirs and journal-only folders sat beside real sessions; each junk name went through `loadSessionMeta` → `resolveSessionId` (full prefix/title scan), so `forge sessions list -n 5` and `forge doctor` paid ~8s on a 280-dir home while `forge stats` finished in 0.5s. Listing now reads `meta.json` for that directory only. Job: after `forge sessions list -n 5`, five rows return in the stats-like class even when junk dirs remain.
- **PATH launcher names a stale `dist/`** (`scripts/link-forge.mjs`): after `git pull`, `~/.local/bin/forge` still exec'd gitignored `dist/cli.js` with no mtime check, so `forge help` was the old auth wall while `npx tsx src/cli.ts help` was Usage. The launcher now prints `bash install.sh` on stderr when `dist/cli.js` is older than `src/cli.ts`, then still execs. Current dist is silent. Job: `forge help` either matches this checkout or the wrapper says how to catch up.
- **Stale `dist/` Next is `bash install.sh`** (`src/util/stale-dist.ts`): PATH `forge help` was `Not authenticated` while `forge --help` was Usage because gitignored `dist/cli.js` lagged `src/cli.ts`. CLI `forge doctor` hygiene rec (not `ok: false`) Next is `bash install.sh` when the running `dist/cli.js` is older than `src/cli.ts`. tsx / `src/cli.ts` omits it. Job: after `git pull`, `forge doctor` tells you to rebuild.
- **Doctor journal Next drains bytes** (`src/session/mutations.ts`, `src/tui/doctor-card.ts`): `forge doctor` warned ~720 MB of `mutations.jsonl` then Next was `forge sessions prune --keep 50`, which keeps lastError sessions and never sees journal files in dirs without `session.json`. Next is `forge sessions prune --journals` (REPL `/sessions prune --journals`): drops undo journals, keeps sessions and lastError. Session-count Next is still `--keep 50`. Job: type the journal Next key, `undoJournal.bytes` go down.
- **Bare `hello` is a run, not help** (`src/cli.ts`, `src/cli/suggest-command.ts`): after `help` became a subcommand, `forge "hello" --json` was `command_typo`→`help` because `hello` starts with `help`. The matcher no longer treats a command as a prefix of English; `helpp` still recovers. `forge run "help"` stays a prompt. Job: `forge "hello" --json` is the same fail-closed run as `forge run "hello" --json`.
- **Stats lastError is the problem Set** (`src/session/metrics.ts`): `forge stats` “By lastError code” / `--json` `byLastErrorCode` counted `ulw_done` from `run_end` while doctor / `sessions errors` already omitted designed wraps. The histogram now uses `isLastErrorProblem`, same Set. Job: after `forge stats`, lastError is the recovery backlog, not 21 finished ULW cycles.
- **Doctor yolo Next persists** (`src/cli.ts`, `src/tui/doctor-card.ts`, `src/config/preferences.ts`): CLI closer was `forge --permission-mode default`, a process-local flag, so the next `forge doctor` was still `bypassPermissions`. Next is `forge permissions default`, which writes the same sticky `preferences.json` as `/permissions`. `--permission-mode` stays one-shot. Job: type the doctor Next key, the following doctor is not still yolo.
- **`forge help` is help** (`src/cli.ts`, `src/cli/help-groups.ts`): unsigned `forge help` was a bare prompt and failed `unauthenticated`; `helpp` never Did-you-mean because `help` was not a subcommand. `forge help` now prints grouped `--help` (exit 0, no auth); `help --json` is `{ reason: "help" }`; `helpp` recovers to `forge help`. `forge run "help"` stays a prompt. Job: type `forge help` without signing in, see Usage.
- **Completion lists what `--help` lists** (`src/util/completion-script.ts`): bash/zsh/fish `cmds=` omitted `lsp` and `tmp` while `--help` listed them; top flags omitted `--max-cycles` / `--max-cost`. Scripts now join `TOP_LEVEL_COMMANDS`. Job: `eval "$(forge completion bash)"` Tab-completes `lsp` and `tmp`.
- **`forge setup` persist fails closed** (`src/commands/setup.ts`, `src/cli.ts`): `persistSetupBudget` / `markProviderModelConfirmed` swallowed `savePreferences`, then the CLI printed “persisted” while `config --json` stayed unlimited. The write now throws; `forge setup budget 5` / `setup model` exit 1 with no “persisted” line when preferences cannot be written. Bare `forge setup budget` peeks and does not write $5. Job: a failed preferences write is an error, not a green card.
- **Designed ULW wraps are not the errors backlog** (`src/session/last-error.ts`): `doctor` / `sessions errors` counted `ulw_cycle_complete` as problems because `LAST_ERROR_OUTCOME_CODES` was only `ulw_done`. Older sessions still stamp the wrap code; it now stays out of the tally with `ulw_done`. `ulw_released` is still a problem. Job: after `forge sessions errors`, the list is 429s and stuck runs, not finished cycles.
- **`forge lsp status` follows the ensure pack** (`src/lsp/manager.ts`, `src/cli.ts`): optional gopls / vscode-* off PATH printed `lsp  ·  missing` and `Next  /lsp ensure` while TS+Python were ready (`toInstall=[]`). Verdict is `missing` only when the ensure pack still has work; idle optional servers still list. CLI Next is `forge lsp ensure`. Job: `forge lsp status` is not missing when recommended servers are on PATH.
- **`forge tmp` default is show, not delete** (`src/cli.ts`): `--help` said “Show or prune” with prune as default, so bare `forge tmp` deleted leftover look/Chrome dirs. Default is now `stats`; `forge tmp prune` still deletes. Doctor Next stays `forge tmp prune`. Job: type `forge tmp`, see sizes; type `forge tmp prune` to delete.
- **`forge setup` verbs persist the checklist** (`src/cli.ts`, `src/config/preferences.ts`, `src/commands/setup.ts`): Next after `forge setup` was `forge --max-cost 5` / `forge config` — a run flag and a snapshot, not a saved budget or model confirm. `forge setup budget 5` now sticks the spend cap in preferences (`forge config --json` `maxCostUnlimited` is false); `forge setup model` drops “(not confirmed)”. CLI Next and `setup --json` `action` values are those shell verbs. Job: type `forge setup budget 5`, the next `forge setup` shows a finite cap.
- **CLI Next keys type at the shell** (`src/util/setup-readiness.ts`, `src/tui/doctor-card.ts`, `src/cli.ts`): `forge setup` printed `Type 1–6` and `/setup skip` then exited; `forge doctor` Next listed `/permissions` (there is no `forge permissions`). CLI cards now use shell verbs (`forge login`, `forge setup`, `forge --permission-mode default`); numbered 1–6 stay on REPL `/setup`. Job: type the Next key after `forge setup` / `forge doctor`, it is a command that exists.
- **Headless unknown slash does not keep a session** (`src/commands/slash.ts`, `src/commands/headless-slash.ts`): `forge run "/helpp"` was Did-you-mean but `classifyLiveSlash` idle-only, so an untitled 0-turn session stayed on `sessions list`. Unknown / Did-you-mean is a readonly probe (ephemeral without `--session`); `/help` stays ephemeral; `/plan` and `--session` still persist. Job: a help typo is not another untitled session.
- **glob/grep of `$HOME` no longer hangs the machine** (`src/agent/tools/search-root.ts`, `glob-list.ts`, `grep.ts`): six overnight Planners globbed `**/forge-*` under `/Users/<name>` and sat in `uv__fs_work` on Chrome's quarantined profile until Chrome was killed. The tools now fail closed on `$HOME` / `/Users` / `~/Library` (YOLO does not walk that tree), skip Library and browser profiles, cap walked entries, and abort after 15s (`FORGE_GLOB_TIMEOUT_MS`). Job: `glob { pattern: "**/*", path: $HOME }` errors in under a second and lists no `Library` files.
- **ULW verify is a process that exits** (`src/harness/cycle/verify.ts`, `declared-checks.ts`, `orchestrator.ts`): `Verify: … && npm run preview` never returned (Vite held pipes; `'close'` never fired; process-group kill was bash-only). Verify now spawns its own group, process-group-kills on timeout, settles on `'close'` so the suite summary drains (on timeout/kill, `'exit'` + a bound wait — orphaned pipes never `'close'`), refuses preview/dev/watch/serve as whole shell words (`serve-check` still harvests), keeps the finite prefix of a compound (`build:ea && preview` → `build:ea`), harvests `mkdir && swiftc HostCareCheck`, and an `.xcodeproj` tree lists `xcodebuild` so a software workspace is not eight cycles with `verifyCommand: null`. Job: word-games' preview is not the gate; pixel-pets' HostCareCheck is.
- **A starved review keeps the look** (`src/harness/cycle/artifacts.ts`, `orchestrator.ts`): QQHX wrote `Verdict: ship` + fulfillment and maxTurns replaced it with 95 bytes of "Reviewer did not complete". `Worth: yes|no` is still required to ship; a labelled look without Worth salvages as blocked with Looked/Fulfillment on `review.md`. Job: QQHX c2 is `reviewVerdict: blocked` with the looked gap, not a stub.
- **Synth Direct execute honours architecture hold** (`src/harness/cycle/orchestrator.ts`): a no-plan synth after two shipped reviews of `select` now `leave it`s the class (three ships prepend a collapse item) so admit cannot ignore the hold. Job: two shipped `select` reviews do not admit a generic Direct execute.
- **Dropped images and Playwright-down are failed looks, not a seen product** (`src/harness/cycle/artifacts.ts`, `src/util/product-kind.ts`, `src/mcp/manager.ts`): `this provider dropped N image attachment(s)` / `Playwright is down` / `Vision dropped` trip look infra; `simctl` is not kernel evidence; watch/glance/HUD/AppKit are surface sits. Playwright prewarm skips Godot/native trees (`playwrightLookApplies`). Job: a watch-face ship whose screenshots were dropped does not commit; a Godot tree does not connect Playwright.
- **Look/compile junk is not a product edit** (`src/agent/tools/bash-mutation-journal.ts`, `src/util/git-ensure.ts`): pixel-pets journaled 13k `.swiftmodule` rows under `.forge/tmp-c3-look`. Those paths (`*.pcm`, `*.o`, `looks/*.mjs`, `look-shots/`) skip the journal; the starter `.gitignore` lists `.forge/tmp-*/`, `*.swiftmodule`, and `*.pcm`. Job: a HostCareCheck compile does not bump `editCount`.
- **Verify timeout reaps session GUI** (`src/agent/loop.ts`, `src/agent/browser-lease.ts`): reap never ran while verify hung, leaving Vite `:4173`, `simctl` glance, native `.build/debug` bins, and a 199 MB look profile. `simctl` / `vite preview` / `npm run dev` / workspace native bins register as GUI leases; reap matches this lease's tokens (simctl bundle id, `--port`, native bin name), never a product name. Verify timeout reaps the session. Job: a hung preview does not leave `:4173` after the gate times out.
- **AGENTS.md Commands fence no longer poisons the ULW verify command** (`src/util/project-intel.ts`, `AGENTS.md`): harvest kept inline `#` comments, then joined the first two lines with `&&`. `/bin/sh -c` treated `#` as a comment, so `&& npm test` never ran and a cycle could go green on typecheck alone. Harvest now strips unquoted `# …`; comments in this repo's Commands fence sit on their own lines. Doctor/`forge run --json` fail when a preferred check still carries a comment. Job: `Verify:` on this repo is `npm run typecheck && npm test`.

### Added
- **Peer scout** (`src/harness/cycle/peer-scout.ts`): while ULW executes, a read-only harness sibling researches 1–3 maintained GitHub peers of this Identity's job and writes a staged `peers.md`. The next Planner weighs it after `Looked:` (bar, not a backlog). The executor and the Reviewer never see it. Fail-open — plan admission does not wait. Stage 3 + stable Identity sleeps; Identity change re-aims. `/cycle status` shows the stage; `/cycle peers` shows the note; the dock paints `peers 2` while it runs. `github` search accepts `sort=stars|forks|updated`. `FORGE_ULW_PEERS=0` off. Job: a long mill knows the category bar without burning the scout on GitHub, and does not cargo-cult a star count.
- **Doctor Recommended + leftover look/tmp + orphan subagents** (`src/tui/doctor-card.ts`, `src/commands/slash.ts`, `src/util/forge-tmp.ts`, `src/session/session.ts`, `src/cli.ts`): doctor prints a Recommended block with sit-down Next keys for leftover `~/.forge/tmp` Chrome/look profiles (`forge tmp prune`), nested `subagent:` sessions with no `ulw.json` (`forge sessions prune --orphans` — ULW parents kept), a max_turns backlog (do not raise explore turns — mills spend more, they do not finish more), and bloated project memory (`/memory project prune`). `forge tmp prune` deletes scratch names older than 6h. Job: type `/doctor`, see the leftover 2.9 GB look profiles and the 100 max_turns mills, and the Next key is the prune, not `/status`.

### Changed
- **ULW packs a Worth: no class-slice in this cycle, and 12 is a backlog not a session** (`src/harness/cycle/artifacts.ts`, `orchestrator.ts`, `briefs.ts`, `skills/forge-planner`, `forge-reviewer`): 5 was the old silent slice, not a measured session. A continue plan of more than 12 items does not parse. Pack the class. `Worth: no` on one item while `Considered:` still holds two or more candidates puts those siblings on this cycle's board instead of paying another Planner (`FORGE_ULW_SLICE_PACK=0` restores skip-and-replan). A packed cycle that was not worth it still does not commit. The Reviewer's scope is the plan's class; Must-fix names what a user would hit. Job: `/ulw` on a CLI hygiene look ships the class in one round, or the Reviewer's Worth: no finishes it on the same board.
- **ULW cycle is a session, not one tiny task** (`src/harness/cycle/artifacts.ts`, `briefs.ts`, `orchestrator.ts`, `skills/forge-planner`, `forge-reviewer`): HashPet forbade one-string-per-cycle; the attractor became one-defect-per-cycle, so a dogfood paid a fresh Planner+Reviewer+suite for each 15-line Next-key slice while `Looked:` still held the class. Default plan is two to five items of one theme (at most 5 — no silent slice). One item is the exception and owes `One item:` (why the other `Considered:` winners are a different job or leave-it, not sequels). A one-item plan with two or more `Out of scope:` entries does not parse — that was the sequel parking lot. Numbered item lines under `One item:` do not parse (they were being dropped). `Out of scope:` is a different job or a lease limit, not cycle N+1. `Worth the cycle:` prices the session (the scout already spent). The record ledger shows item counts so a one-item mill is visible; a one-slice cycle whose `Considered:` already listed the class is Reviewer `Worth: no`. Job: `/ulw` packs the class it just typed into one executor round, not twenty expensive one-item cycles.
- **Planner briefs stop sending the model hunting for `forge-*` on disk** (`src/harness/cycle/briefs.ts`, `skills/forge-planner/SKILL.md`): the inlined category skill is the playbook (`Category skill inlined: forge-game-assets`). Briefs say do not glob/grep `$HOME` for skill files; `web_search` is the market bar. Job: a game Planner does not glob `/Users/<name>` for `**/forge-game*`.
- **Foreground bash timeout default is 180s** (`src/util/env.ts`): a suite that targets "under 2 min" used to be SIGTERM'd at the 120s default. `FORGE_BASH_TIMEOUT_MS` still overrides. Job: `npm test` via bash is not killed at 2:00 on a slow machine.
- **ULW: one category skill from the tree** (`src/util/product-kind.ts`, `src/agent/loop.ts`, `src/harness/cycle/briefs.ts`). Planner and Reviewer already inline forge-planner/reviewer + veteran + rootcause; they now also inline the matching category skill — CLI `forge-shape`, Godot/extension `forge-game-assets`, web `forge-surface`, library `forge-prove` — detected from the tree, never the prompt. The scout brief names it (`Category skill inlined: forge-shape`) so the role knows it is already in context. Job: `/ulw` on a CLI inlines forge-shape; a Godot tree inlines forge-game-assets; the user did not have to name the skill.
- **ULW: no-commit wall and Worth: no Direction hold** (`src/harness/cycle/state.ts`, `orchestrator.ts`, `artifacts.ts`). Three cycle closes with nothing committed (blocked / unparseable review, surface-sit skip, empty skip, docs-only) share the no-progress wall (`noCommitStreak`, same `FORGE_ULW_NO_PROGRESS_CAP`); a substance commit resets it, and `FORGE_ULW_AUTO_COMMIT=0` after a green ship is progress. After a shipped `Worth: no`, the same `Direction:` does not admit — one retry, then go-deeper unless promises are unkept (`FORGE_ULW_WORTH_HOLD=0` off). Job: three blocked reviews do not start a fourth plan; after Worth: no, the same Direction is not a plan.
- **ULW: never skip the record after cycle 1** (`src/harness/cycle/orchestrator.ts`, `briefs.ts`). Scout-as-plan skips the plan turn only with an empty ledger. The plan brief keeps eight full cycle lines; older rows collapse to title · review · commit. Job: after cycle 1 the Planner always reads the record; the ledger keeps eight full cycle lines.
- **ULW: parse the contract the brief reprints** (`src/harness/cycle/artifacts.ts`). A `continue` plan without nonempty `Looked:`, `Direction:`, and `Worth the cycle:` does not admit (a failed sit — "could not run — why" — is a look; an empty Looked is not). A review without `Worth: yes|no` does not approve. A scout without `Looked:` is incomplete. Same retry-then-synthesize path as missing `leave it` / `serves:` / `red now:`. Job: a continue plan without Looked/Direction/Worth the cycle does not admit; a review without Worth: yes|no does not approve.
- **ULW: kickoff, admit, and README match the driver lifecycle** (`src/harness/cycle/index.ts`, `src/harness/context-admit.ts`, `src/harness/cycle/briefs.ts`, `src/agent/system-prompt.ts`, `src/commands/slash.ts`, `README.md`, `docs/ULW.md`, `skills/forge-reviewer`). Unlimited copy no longer says a no-mandate run ends when the product looks fine — release is a mandate `fulfilled`, Planner `blocked`, `/cycle 0`, `max_cycles`, or the no-progress wall. Reviewer turn 2 is the document; Must-fix is how the tree changes. Job: kickoff/admit/README no longer tell a no-mandate run it can stop because the product looks fine; Reviewer turn 2 is the document.
- **The suite is in-process and finishes in about two minutes** (`tests/reliability.test.ts`, `package.json`, `src/util/retry.ts`, `src/util/process-tree.ts`). `npm test` no longer boots `dist/cli.js` a hundred times (that is `npm run smoke`); kill-grace / retry / ULW auto-continue waits are `FORGE_KILL_GRACE_MS=50` / `FORGE_RETRY_BASE_MS=1` / `FORGE_ULW_AUTO_CONTINUE_BASE_MS=1` under the test script. Job: `npm test` is the contract lock you run after every edit, not a five-minute tax.
- **ULW salvages labelled plans/reviews the overnight mill threw away** (`src/harness/cycle/artifacts.ts`, `orchestrator.ts`, `roles.ts`, `state.ts`, `status.ts`). Six dogfoods (魔塔 / QQT / Pixel Pets / Chain Bench / QQHX / Yardfowl) blocked on parse stubs and synthesized Direct execute. `Verdict: **ship.**` and a labelled plan/review after `[Forge] maxTurns` salvage — `Goal achieved.` still does not approve. Architecture hold is `leave it` that names the class, or `Out of scope:` that names it. A starved Planner's synth copies the scout's candidates and keeps the existing gate. Reviewer document retry uses the look budget (25), not the Planner's 12. A substance commit resets the no-progress wall; `synthStreak` still caps. Dock shows `synth N/3`. Job: the next overnight `/ulw` admits the plan the scout already wrote, and a Reviewer that wrote `Verdict:` is not discarded.
- **`limited` promises, MEMORY replace, Out of scope persists** (`src/harness/cycle/artifacts.ts`, `state.ts`, `orchestrator.ts`, `src/harness/project-memory.ts`, `src/agent/loop.ts`). Lease/platform evidence (TCC `-10004`, EPERM, `LS 115`, Playwright never initialized) is `limited`, not a keep-promise hole. `Promise:` / `Identity:` rows replace, not append. Cycle 1 `Out of scope:` survives a later empty synth plan. Job: a watch face the lease cannot tap does not refill Direct execute.
- **A kernel/API look is a look; `/cycle 0` can skip a dead one** (`src/harness/cycle/artifacts.ts`, `orchestrator.ts`). `walk()`, `--self-test`, `SMOKE_*`, `simctl` still used the product when the GUI lease died — `npm test` / `cargo test` did not. `/cycle 0` with items done does not block a labelled `Verdict:` on a dead look. Job: 魔塔 cycle 11's `Verdict: ship` with a `walk()` tape is not discarded because Playwright was down.

### Added
- **Bottom dock shows folder and git branch** (`src/tui/bottom-status.ts`, `src/statusline/render.ts`): the sticky status line and the dock-off compact strip now paint the working directory (last two path segments) and `git:<branch>` (`*` dirty, `+wt` worktree) next to the brand, so a multi-project / multi-branch REPL is identifiable without `/status`. Job: glance at the dock and know which repo and branch you are in.
- **Auto-init git when a new project needs a repo** (`src/util/git-ensure.ts`): ULW used to skip the cycle commit with `not a git repository` and keep planning. A software workspace (or any ULW commit) that is not already inside a repo now gets `git init -b main` and a starter `.gitignore` before the commit — local only, never nested in another repo, never `$HOME` / `/` / the OS temp dir. `FORGE_AUTO_GIT=0` off. Job: `/ulw` a greenfield app, the first reviewed cycle commits instead of skipping.
- **`github` tool** (`src/agent/tools/github.ts`): search repositories/code, repo metadata, file contents, README, tree. Public repos work without a token; a local `gh auth login` is picked up via `gh auth token` (also `GITHUB_TOKEN` / `GH_TOKEN`). Tokens never enter the output. `web_search` on a github.com / `owner/repo` query fans out to GitHub search automatically, and web hits that are github.com URLs name the repos to read with the github tool — the user does not have to say "search GitHub". `web_fetch` rewrites `github.com/.../blob/...` to `raw.githubusercontent.com`. `FORGE_GITHUB=0` off. Job: read a library's source instead of googling its marketing page.
- **Web search provider chain** (`src/agent/tools/web-search-providers.ts`): Brave / Tavily / Exa / SearXNG when their keys/URL are set; otherwise DuckDuckGo Instant Answer plus DDG / Brave / Bing HTML in parallel, merged and de-duplicated. Two-minute in-process cache. Job: a technical query still returns hits when DuckDuckGo Instant Answer is empty.

### Fixed
- **A CLI with a Vite config is still a CLI** (`src/util/product-kind.ts`). `vite.config` / `next.config` no longer outrank `package.json` `bin`, matching the `index.html` heuristic. Job: a CLI that ships a docs playground still inlines `forge-shape`.
- **`limited` promises do not synthesize keep-promise** (`src/harness/cycle/orchestrator.ts`, `briefs.ts`). Lease/platform `limited` already dropped out of keep-promise items; the kind picker and the Planner's "unkept = default next job" list still treated them as holes. Job: a watch face the lease cannot tap does not refill Direct execute when the Planner writes `fulfilled`.
- **xAI `fetch failed (UND_ERR_INVALID_ARG)` on Node 24+/26** (`src/providers/http-agent.ts`): npm `undici@6` Agent was passed to Node's global `fetch` (bundled undici 8). HashPet `052af525` retried `invalid onError method` eight times and never reached the API. Chat uses the same-package `undici.fetch` + Agent. Job: resume a long xAI session, the first round is a chat, not a retry storm.
- **Typing no longer recomputes the dock on every key** (`src/tui/bottom-status.ts`, `repl.ts`): editor `onPaint` called `refresh()`, which ran `git status`, walked the whole transcript for a token estimate, and re-read `ulw.json` / accounts on every keystroke and every 200ms spinner tick. Restore restamps the last painted line; recompute stays on the 2s timer. Job: type at `forge ›` on a 400-turn session, keys keep up.
- **Native select and copy-on-select work again** (`src/tui/mouse.ts`): SGR mouse tracking defaulted on and stole drag-select from the terminal. Click-to-caret / dock chips are `FORGE_MOUSE=1`. Job: drag a stack trace, it copies.
- **Prompt caret and wrap use terminal columns, not JS string length** (`src/util/cell-width.ts`, `src/tui/prompt-editor.ts`, `src/util/format.ts`): `你好` is 4 columns; mixed `hi你好` no longer parks the caret two cells left of the last glyph. Backspace deletes a whole 👍 / 你; `clipAnsi` / markdown wrap / the dock git chip hard-break on columns so a Chinese folder name cannot overflow the last row. Job: type `请修复 dock 闪烁`, the caret sits at the end.
- **Bottom dock is not eaten by `live ›`** (`src/tui/prompt-editor.ts`, `src/tui/bottom-status.ts`): the editor erased with `CSI J` (end of display), which ignores DECSTBM and wiped the reserved row every 200ms spinner tick; the dock then always DECSC-repainted even when the line had not changed. Erase is now per-line EL; identical dock paints are skipped; each editor redraw restores the dock in the same frame. Empty Enter while busy does not stack blank `live ›` rows. Job: a long think does not blink the folder/git/model line away.
- **Click-to-caret and dock chips** (`src/tui/mouse.ts`, `src/tui/prompt-editor.ts`, `src/tui/bottom-status.ts`): SGR mouse is opt-in (`FORGE_MOUSE=1`; press only; Shift-click ignored) so a default REPL keeps native select/copy. Click in the prompt places the caret on a grapheme, including the right half of 你. The dock `stop` chip aborts the turn (same as Ctrl+C, not `/cycle 0`); after abort, `resume` is `/retry`; model / ULW / auth chips open `/model` / `/cycle` / `/accounts`. Job: `FORGE_MOUSE=1`, click `stop` while it thinks; click in `hi你好` to sit on 好.
- **Agent bash does not stamp the cycle gate while ULW is armed** (`src/agent/loop.ts`): `lastVerificationCommand` is the harness gate, not an executor `npm test`. Job: a cycle's verify command stays the Planner's `Verify:`.
- **Guideline audit ignores `windows/` dump roots** (`src/harness/guideline-audit.ts`): a port tree that cites dump paths is not a stale-path mill. Job: QQHX `windows/data/Res/foo.fsp` is not a missing project file.
- **Session-owned resource leases, not a Chrome-prefix regex** (`src/agent/browser-lease.ts`, `src/util/look-cleanup.ts`, `src/agent/tools/bash.ts`): bash Chrome/Godot is leased on the **root** session (child UDDs no longer vanish when the Planner dir is deleted). `/tmp/hearth-*` and nested `/tmp/mom-c20/cdp-profile8` register; SIGTERM is followed by SIGKILL; a failed kill keeps a zombie row instead of wiping `browsers.json`. `/cycle 0` / `/ulw-off` reap while the REPL stays up. A harness look profile is minted per session (`browsers/look`) so roles reuse one UDD. `FORGE_BROWSER_REAP=0` still skips. Job: four concurrent `/ulw` + `/cycle 0` leave no Chrome-for-Testing or Godot `--write-movie` behind.
- **`/cycle 0` does not write the next scout** (`src/harness/cycle/orchestrator.ts`): after a closed cycle the overlay now aborts the Planner before `scout.md` is written and reaps look processes. Job: type `/cycle 0` after a blocked review, no cycle-N+1 scout and no extra Chrome.
- **Reviewer turn 2 is report-only** (`src/harness/cycle/orchestrator.ts`, `roles.ts`): the look may drive the product (25 turns); the review document cannot edit 10 files and then fail to parse. Job: Hearth cycle 1's veteran prose cannot mutate the tree without a `Verdict:`.
- **Look infra is not "look the surface"** (`src/harness/cycle/artifacts.ts`): maxTurns / Godot crash / Chrome-for-Testing died is a different must-fix so the next Planner reuses the lease instead of planning another first-hour paint from source.
- **Proactive 98% switch no longer stampede concurrent TUIs** (`src/auth/accounts.ts`): a pinned session stays on its slot until 429/cooldown; pin restores `active` without jumping every dogfood onto one SuperGrok. Job: four xAI ULWs do not all failover to `chestnutp426` because one hit 98%.
- **`fetch failed` retries record `error.cause`** (`src/util/retry.ts`); xAI chat uses a dedicated undici Agent (`src/providers/http-agent.ts`, 32 connections) so Playwright/MCP do not share the default pool. Job: the next HashPet drop says `ECONNRESET`, not a bare TypeError.
- **HARD DENY of `auth.json` is a tool error, not a red TUI crash** (`src/agent/permissions.ts`, `loop.ts`, `protected-paths.ts`): credential refuses stay in the tool result; grep of `~/.forge` names sessions/logs/tmp. Job: a Maze look grep of forge-home is not `✖ HARD DENY` in the dock.
- **Cursor drops images out loud** (`src/providers/cursor.ts`): `messageText` used to strip `image_url` parts silently; the model now sees that Cursor has no multimodal parts. Job: a Reviewer look that `read_file`s a PNG does not pretend it saw the pixels.
- **`bash install.sh` survives a second laptop** (`install.sh`, `scripts/link-forge.mjs`): `npm link` under `set -e` used to abort the whole script (EACCES / EEXIST / a prefix that is not first on PATH), and a leftover `forge` (nvm vs Homebrew vs `/usr/local`) stayed hashed. The script now loads nvm/fnm/volta/asdf when Node is missing or too old, rebuilds, then links with fallbacks (`npm link` → `npm install -g .` → a launcher in the npm prefix and `~/.local/bin`), never overwrites a foreign `forge` (Foundry), replaces a stale forge-agent bin, writes a `forge-agent` alias when `forge` is taken, and prints `hash -r` / `export PATH=…` when this shell would still see the old one. Job: `git pull && bash install.sh` is the update; the next `forge` is this clone.
- **A 1×1 / failed-screenshot PNG cannot reach the provider** (`src/util/user-images.ts`, `src/agent/loop.ts`): `read_file` used to emit `[[image:]]` for any png, including an 87-byte CDP leftover, and xAI 400 (`Image dimensions 1x1 are too small`) is `bad_request` — unattended ULW died. Images below 8px are not attached; a dimension 400 drops outbound `image_url` parts and retries once. `FORGE_VISION_MIN_EDGE` for tests. Job: a leftover-door sit does not kill the run.
- **Provider retries are in the session round log; a crash run still records spend; leftover background-task dirs expire** (`src/session/prompt-cache.ts`, `src/session/metrics.ts`, `src/agent/loop.ts`, `src/agent/tools/background-tasks.ts`): HashPet fetch-failed retries were TUI-only, crash `run_end` logged $0, and `~/.forge/background-tasks/` never expired. Retries ride `provider_round` in `rounds.jsonl`; two fetch-failed / ECONNRESET / `terminated` drops in one chat budget reap session browsers; `ok: false` run_end keeps session token totals; settled bg-task dirs older than `FORGE_BG_TASK_TTL_MS` (24h) are removed on run end and cycle commit. Job: the next HashPet fetch-failed storm is visible after the TUI is gone.
- **Bash-spawned Chrome profiles under /tmp and .forge/chrome-cft* are leased and reaped on cycle/error/exit, not only Playwright MCP dispose** (`src/agent/browser-lease.ts`, `src/util/look-cleanup.ts`): Playwright MCP never came up so agents spawned Chrome for Testing under `/tmp/hashpet-*` and `/tmp/mom-*`; look-cleanup only owned `~/.forge/tmp` and `chrome-look*` and only ran on process exit. Every UDD bash spawns is a session lease (`browsers.json`, cap `FORGE_BROWSER_LEASE_MAX`) and is reaped on cycle commit, provider error, and exit (`FORGE_BROWSER_REAP=0` skips the kill). Job: a HashPet look-loop leaves at most one Chrome per session.
- **Quota failover waits out a short account cooldown instead of killing the run; personal-team-blocked is a team spend cap, not a missing login.** Concurrent sessions pin an account so they do not stampede `auth.json` active. `FORGE_ACCOUNT_COOLDOWN_WAIT_MAX` (default 180s, `0` = no wait, cap 900s). Job: HashPet’s 3-minute sning cooldown does not end a 40-hour ULW.
- **The suite no longer follows the parent TTY's colour** (`package.json`, `tests/helpers/pin-color.ts`): Grok sessions export `NO_COLOR=1` (markdown / run-report / statusline / turn-open fail) and a coloured TTY plus `FORCE_COLOR=1` put SGR in the middle of labels (`✗ write secret.txt`, `/model`, picker `1 `). `npm test` now unsets `NO_COLOR` and sets `FORCE_COLOR=1`; colour tests import the pin first; content assertions strip ANSI. Job: `npm test` is the same on a Grok session, a pipe, and a colour TTY.
- **Cursor `hooks.json` no longer crashes Stop** (`src/harness/hooks.ts`): Cursor's native entries are `{ command, matcher?, timeout }` with no `hooks[]`, and the compat loader iterated `m.hooks` — `TypeError: m.hooks is not iterable` on a machine with `~/.cursor/hooks.json` (11 loop tests, documented as a known baseline). Native entries normalize to Forge matchers; user-home Claude/Cursor files are skipped under `node:test` so the developer's hooks cannot steer the suite. Job: `npm test` on a Cursor machine is the suite, not the host's hooks.json.
- **`/cycle 0` during the next Planner actually stops** (`src/harness/cycle/state.ts`, `orchestrator.ts`, `controls.ts`): maze dogfood `93d72272` typed `/cycle 0` after cycle 18 committed, while cycle 19's Planner was running. The slash wrote `cycleZeroRequested` to `ulw.json`; the orchestrator still held the object it loaded before the keystroke and `saveCycleState` clobbered the flag, so cycles 19–21 started. Live controls (`/cycle 0`, `/max-cycles`, `/plan`, `/ulw-off`) now overlay from disk on every orchestrator persist and after Planner / Reviewer / verify awaits. After a closed cycle, `/cycle 0` / `max_cycles` release without admitting another plan; `/ulw-off` aborts immediately (no next plan, and no commit if the Reviewer was still in flight). `/cycle 0` during the first Planner still admits that plan as the last cycle. Job: type `/cycle 0` while a Planner or Reviewer is running, the run stops.
- **Playwright MCP no longer leaves Chrome profiles and screenshots in the repo** (`src/mcp/defaults.ts`, `src/util/look-cleanup.ts`): default args are `--isolated` and `--output-dir` under `~/.forge/tmp/playwright-output` (also applied to an existing `~/.forge/mcp.json` playwright entry unless `--user-data-dir` is set; `FORGE_PLAYWRIGHT_ISOLATED=0` off). MCP dispose and process exit delete `.forge/chrome-look*` profiles, wipe that output dir, and SIGTERM Chromium whose cmdline cites those paths. Unreferenced look artefacts are moved to `~/.forge/sessions/<id>/looks` on the cycle commit. Job: a play-loop leaves the tree without orphan Chromes or PNG dumps.
- **Low-hanging fruit then "done"** (`src/harness/cycle/orchestrator.ts`, `briefs.ts`, `git-auto-commit.ts`): a mandate `fulfilled` with no `Looked:` and no committed cycle yet becomes a go-deeper cycle, not a release; docs-only commits do not reset the no-progress wall; the Planner's record names the files each cycle committed and says not to plan the easiest remaining string. Job: a README-and-hello-world slice cannot end `/ulw build this app`.

### Changed
- **ULW: admit the plan the scout already wrote, keep the product gate, own the commit** (`src/harness/cycle/orchestrator.ts`, `roles.ts`, `artifacts.ts`, `commit-lock.ts`, `src/harness/verification.ts`, `src/util/project-intel.ts`). A native-app `/ulw` died on the no-progress wall while `cycles/15/scout.md` already had the next veteran cycle. (1) A scout that parses as a `continue` plan is admitted; the plan turn stays `documentOnly`. (2) `**Verdict.**` parses as `Verdict:`. (3) A plan-less Planner with unkept promises synthesizes **keep-promise**, not Direct execute, and inherits `Verify:`. (4) A `cargo test -p … && godot --headless --script` compound stays the gate; AGENTS.md Commands are harvested. (5) `git commit` is denied while ULW is armed (`FORGE_ULW_DRIVER_COMMIT=0` off). (6) Architecture hold on a synthesized cycle is a Considered `leave it`, not fake item 0. (7) An unparseable review retries once. (8) Lowercase `godot …` leases into the existing GUI sidecar. Job: a native-app `/ulw` keeps going on the door the scout already named.
- **Architecture class at three shipped reviews must collapse** (`src/harness/cycle/artifacts.ts`, `orchestrator.ts`): last-two consecutive still holds; a class named in three shipped reviews anywhere in the run cannot be `leave it`'d — the plan owes an item. Synthesized direct-execute cycles cap at 2 (`FORGE_ULW_SYNTH_CAP`) even when they commit, so a first-hour mill cannot reset the no-progress wall forever. Job: Ashen cannot ship HUD-clip cycle 16 as a third of the same class.
- **Cycle verify "green vs baseline" is loud** (`src/harness/cycle/status.ts`): `/cycle status` and the run report say `green vs baseline (N pre-existing still red)` instead of a bare green. Job: Hearth's 12 inherited pins stay visible on every cycle card.
- **Look brief names the leased profile and a Godot recipe** (`src/harness/cycle/briefs.ts`): Playwright ready → `call_mcp`, never bash Chrome; a `project.godot` workspace gets OpenGL3 `--write-movie --quit-after`, not `open -a Godot`. Job: the scout does not invent `/tmp/hashpet-c15-reviewer-udd`.
- **ULW: a mandate cannot fulfil while broken/unknown promises have no Operator:** (`src/harness/cycle/orchestrator.ts`, `state.ts`, `artifacts.ts`, `run-report.ts`). Architecture recurrence cannot be ignored for another slice; `/ulw` continue starts a new report epoch. Job: mcp-codex-web cannot declare ChatGPT-attach done from loopback fixtures alone.
- **Look path: Playwright is pre-warmed; a surface-claim cycle cannot commit on a ship whose look never opened the product** (`src/mcp/client.ts`, `manager.ts`, `src/harness/cycle/orchestrator.ts`, `artifacts.ts`, `briefs.ts`). MCP initialize hard-capped at 30s while Playwright's recipe is 120s, so HashPet/Maze reviewers wrote `Looked: Playwright MCP never initialized` and shipped first-hour claims anyway. Initialize now uses the server timeout (`FORGE_MCP_INIT_MS` override); Playwright is connected in the background at session start (fail-open, first keystroke waits at most ~3s). A `ship` whose plan is a surface sit (popup / first-hour / browser / door) and whose look never opened the product is an incomplete review — no commit; `FORGE_ULW_LOOK_GATE=0` off. CLI/JSON-RPC proofs still ship. Job: HashPet c42 cannot ship a Stay-dock change without opening the popup.
- **ULW: the user's prompt is attention, not a quality ceiling** (`src/harness/cycle/briefs.ts`, `artifacts.ts`, `src/agent/system-prompt.ts`, `skills/forge-planner`, `forge-veteran`, `forge-reviewer`) — a laundry-list `/ulw make it more interesting, attractive, addictive, ship-ready` was being treated as a spec the Planner could tick or paraphrase, so prompt quality set work quality. The harness still does not rewrite or classify the mandate (the retired `ulwExpandedMandate` / `isSoftPrompt` path stays gone: a `/` in "UI/UX" once picked the wrong contract for a 250-wave run). What is now in every Planner brief, and inlined in the role skills: the mandate is attention toward a job; vague or hype wording does not license vague or hype work; a specific request is still that request, done like a veteran, not a product rewrite they did not ask for; `Direction:` is the Planner's sentence after using the product and knowing the category's bar (`web_search` it, or read the matching shipped `forge-*` skill — games, UI, CLI); an explicit mandate is fulfilled when the job they pointed at is met at veteran quality, not when every adjective is ticked; the Reviewer judges a demanding user of this product, not whether the diff matches the user's adjectives; the executor ships the plan at veteran quality and does not widen scope because the mandate was vague. Job: type a sloppy `/ulw`, get veteran cycles; type `add --version with a test`, get that, properly, then fulfilled.

### Fixed
- **ULW review approval belongs to completed work**: failed or incomplete reviews cannot authorize a commit; unresolved `Must-fix` and partial/missing items require repair within `fix_rounds`; executor repairs invalidate approval and receive a fresh review. Changing the verify gate cannot adopt failures from a rejected or dirty tree as its baseline. Job: the local commit contains work that passed both verification and completed review.
- **ULW preserves uncertainty and judges lasting value**: `unknown` promises survive parsing, planning, status, and reporting and prompt investigation before repair. Planner, Reviewer, executor, and fallback plans now weigh product-specific reliability, security, accessibility, performance, recovery, and maintenance alongside visible improvements; meaningful regression coverage does not require an unnecessary production edit. Promptless continuation instructions match the driver. Job: unattended work improves what matters to this project without manufacturing edits to fill cycles. Design findings and limits: `docs/ULW-AUDIT.md`.
- **`npm test` no longer clobbers the OS clipboard**: `tests/reliability.test.ts` called `copyToClipboard("forge-clipboard-test")`, which on macOS is a live `pbcopy`. `copyToClipboard` now no-ops under `node:test` / `FORGE_CLIPBOARD=0` (`FORGE_CLIPBOARD=1` to force). Job: run the suite, your pasteboard stays yours.
- **Cursor accounts show the login email**: browser Cursor JWTs have no `email` claim, so `forge accounts` labeled every slot `cursor:subscription` and you could not tell which Cursor user was active. Login, token refresh, `forge accounts` / `forge auth`, and `/accounts` / `/auth` now probe `DashboardService/GetMe` (fail-open, 4s) and store `cursor:you@email.com` the way SuperGrok stores `grok:you@email.com`. Existing `cursor:subscription` rows are backfilled when you list accounts. `--label` still wins over the probe. Job: `forge accounts` names the Cursor user.

### Changed
- **ULW: an unlimited run does not stop on the model's judgement, and its Planner stops wasting turns** (`src/harness/cycle/orchestrator.ts`, `state.ts`, `status.ts`, `src/agent/loop.ts`, `subagent.ts`, `skills/forge-planner`, `forge-reviewer`) — from a HashPet dogfood (`7067fd32`, `/ulw` no mandate) that ended `blocked` after **one commit**. Two diseases, one shape: the model judging its way out of work. (1) **The cycle-3 Planner burned all 60 turns and never wrote a plan**, so `planNextCycle` released the whole run. Cause: the Planner ran `npm test` (being the product's user), it was red, and the harness injected *"fix until green … fix the root cause … continue until the check passes"* into a role that cannot edit (`denyEdits`) — it spent ~45 of 60 turns chasing a failure it structurally could not touch. Fix: `fix-until-green` and the mid-loop verify-nudge are now gated on the loop's own tool set (`toolSetCanEdit(baseToolDefs)`) — a role that owns no edit tool is never told to fix — and the Planner's **plan turn is `documentOnly`** (report-only): the scouting is done on turn 1, so turn 2 emits the plan and cannot re-enter reading. (2) **A plan-less Planner no longer ends the run.** It becomes a **direct-execute cycle**: the harness synthesizes a one-item plan from the scout's findings and hands it to the executor (which can edit). (3) **A no-mandate `fulfilled` no longer ends the run** — "the product is in good shape" on a broken product is the same escape. It becomes a **keep-promise** cycle when any promise is `broken`/`absent` (a broken flow, a screen with no way back), or a **go-deeper** cycle when every promise is genuinely `kept`. A **mandate** `fulfilled` still releases. The only self-stops left are a mandate fulfilled, a Planner `blocked`, the user's controls, and the **no-progress wall** (`directExecuteStreak` ≥ `FORGE_ULW_NO_PROGRESS_CAP`, default 3, reset by any commit) — an honest floor for a run that lands nothing, honouring non-negotiable 2 without giving the model a way to quit by declaring victory. (4) **Doctrine so brokenness gets noticed**: `forge-planner` / `forge-reviewer` now require driving the *whole first session* — navigating into and back out of every screen, not stopping at the first (the run that called HashPet "in good shape" had only ever looked at the first screen; the broken navigation was one click past where it stopped) — treat navigation as a promise, an undriveable flow as `UNKNOWN` never `kept`, and forbid pronouncing a product good from a static read of source or screenshots. Job: `/ulw` with no prompt keeps improving the product — perpetually, until you stop it — instead of reading for an hour and then telling you a broken product is excellent.
- **ULW: the roles run in two turns and carry the doctrine at the run level** (`src/harness/cycle/roles.ts` new, `briefs.ts`, `artifacts.ts`, `orchestrator.ts`, `state.ts`, `status.ts`, `src/agent/subagent.ts`, `loop.ts`, `system-prompt.ts`, `skills/forge-planner`, `forge-reviewer`, `forge-veteran`). Read from the same 30-cycle HashPet run (`7e705c79`) one level deeper: cycle 21's review *named the root cause* of the rename run — "Captured is still three independent strings — same split Ate had; pre-existing, left alone" — filed it under `Architecture`, and eight more renames shipped. The doctrine that would have stopped it (`forge-rootcause`: no fix without root cause; `forge-shape`: alternatives, never one answer as consensus; `forge-redgreen`: a proof that already passes is not a test; `forge-prove`: run, read, then claim; `forge-assay`: a checklist against current state) was written for a task and never applied to the cycle or the run — and the Planner brief handed two thousand characters of record *before* the procedure that said "use the product before you read a line of source". None of what follows is a meter. (1) **Two turns of one kept session per role.** The Planner's turn 1 (the **scout**, `cycles/<n>/scout.md`) has the product, the tree and the promises as last recorded — and nothing of the record; it uses the product and writes `Identity:`, `Looked:`, `Promises:` (the product's own claims — README, `--help`, tests as spec — each `kept | broken | absent` as inspected) and `Considered:` (one candidate per gap bin plus `leave it`). Turn 2 (the **plan**) resumes the same session with the record, the executor's lines, the user's words and the run's **spend**, and writes `plan.md`. The Reviewer's turn 1 (the **look**, `look.md`) has the product on the tree as the cycle left it and no diff; turn 2 the diff. The role that used the product is the one that writes the document; a parse retry re-enters turn 2 only, so the contract can be strict without re-scouting. `SubagentRequest.keepSession` keeps a clean child for the second turn; the harness removes it when the document is in hand; a resumed turn reports the child's cumulative usage, so the role's tokens on the cycle record are the larger figure, not the sum (`roleTokens`). `FORGE_ULW_TWO_TURN=0` runs one brief per role; a runtime that cannot keep a session, or a resume that fails, falls back to the single brief with the scout inlined — nothing here releases a run. (2) **The plan contract owes the doctrine's lines**: a `continue` plan needs `Considered:` with a `leave it` entry (the null hypothesis every cycle beats in writing), `Worth the cycle:` (why this beats leaving it for the user in `Identity:`, and why it is from *this* product — the `forge-surface` self-critique), and every item carries `serves:` (the job it serves, in words) and `red now:` (what the Planner ran or saw that shows it is not yet so; `unchecked — why` is a valid line); a plan missing any of them comes back once with what was missing named (`explainPlanParseFailure`), then `blocked` as before. (3) **The class rule, in both roles, with `forge-rootcause` inlined beside `forge-veteran`**: the same kind of change the record shows twice is a symptom — the Planner plans the one decision (the shared module, the one vocabulary) or leaves it; the Reviewer's brief now carries the run's record and its duty says the root cause goes under `Must-fix`, not `Architecture`, with `Verdict: blocked — the run is patching symptoms` at three. (4) **`Promises:` are the run's checklist** (`ulw.json` `promises[]`, project memory `Promise:` rows beside `Identity:`): the next scout is handed them to re-inspect, never to copy; `/cycle status` tallies `kept · broken · absent`; the run report lists broken and absent promises under **Not done**; `fulfilled` means kept. (5) **Claim beside finding**: the Planner's `Worth the cycle:` sits next to the Reviewer's `Worth:` on the cycle line the next Planner reads (`worth claimed: … · worth found: …`), in `/cycle status` and in the report's **What shipped**, so whether the boss's sentence held is readable per cycle. (6) **The executor may dispute with evidence** (`forge-absorb` reconciled with `forge-veteran`): a `Dispute:` line in its closer — a Reviewer revision it can show was wrong — is harvested like `Serendipity:` and reaches the next Planner. Also: the Reviewer's look and review are the `forge-prove` order (run, read, then claim), the Reviewer reads the plan's `Considered:` and says under `Worth:` whether the Planner chose right, and the Planner's explore children get lenses (first-minute user, month-three user, next year's maintainer, a competitor's PM). Job: a `/ulw` run plans from the product it just used, has to say in writing why this cycle beats leaving it, and stops patching a symptom the moment its own record shows the class twice.
- **ULW: the executor hears the review, the Planner hears the executor, the baseline follows the gate, and nobody waits at the keyboard** (`src/harness/cycle/briefs.ts`, `orchestrator.ts`, `artifacts.ts`, `state.ts`, `src/agent/tools/index.ts`, `ask-user.ts`, `src/agent/system-prompt.ts`, `skills/forge-veteran`). Read from the plan-cycle driver's own record rather than a run: the review had two readers — the next Planner and the run report — and not its author. (1) **The plan admission after a reviewed cycle carries *The Reviewer's notes on cycle N — standing for this run***: verdict and `Worth:`, `Revisions` (what changed in the executor's work and why), the items the Reviewer judged partial or missing against the executor's board (`disputed` on the cycle record), the `Architecture` notes, and the standing instruction that the Reviewer should not have to make the same revision twice; a review that changed nothing says so in one line. The Reviewer's brief carries the previous review's shape notes ("is any of it back?") so a recurrence is a `Must-fix`, not a rediscovered note. (2) **`Serendipity:` lines reach the Planner.** The executor protocol promised them "for the Reviewer and the next Planner" and only the Reviewer's brief had the closer; every stamped Stop now harvests the labelled line (same-line remainder, or bullets under a bare label — `extractSerendipityLines`) into the cycle record, and the Planner brief lists them newest cycle first as evidence to weigh. (3) **The verify baseline follows the gate command.** It was captured at cycle 1 only and keyed to that command; a Planner that declared `npm run check` after cycles gated by `npm test` — or the first cycle to declare a check at all — met `judgeAgainstBaseline` with "no baseline" and every pre-existing failure became the executor's to fix, the exact three-fix-round failure the baseline was built to end. An admission whose gate differs from the baseline's command runs it once on the tree as the last commit left it (`cycles/<n>/verify.baseline.log`). (4) **`ask_user` fails closed while the driver is armed** — on the executor and on every session under the armed one, the Reviewer and the Planner's explore children included (the gate follows `subagent.parentId` to the root, not one hop) — with the protocol's instruction (decide it, record the assumption; only a secret / irreversible action / external blocker is an `Operator:` line). In a TTY the tool waited `FORGE_ASK_USER_TIMEOUT_MS` (5 min) per question for nobody. Under a human `/plan` the user holds the keyboard until `/build`, and the tool works as usual. Job: a `/ulw` run left alone gets better at its own craft as it goes, plans from what the work itself turned up, keeps its gate honest when the Planner changes the check, and never sits waiting for a person who is not there.
- **Add-account commands sit where you look**: `./install.sh` Next notes, bash/zsh/fish completion headers (`eval "$(forge completion bash)"`), `forge login --help` / `--add`, `/accounts`, `forge tips`, GETTING-STARTED. Cursor needs `forge login -p cursor --oauth --add` so it does not re-import the local session. Job: forget how to add a second Cursor account, the card still has the line.
- **ULW: the Planner uses the product, the record is not a thread, the Reviewer judges worth, "leave it" is a verdict** (`skills/forge-planner`, `skills/forge-reviewer`, `src/harness/cycle/briefs.ts`, `artifacts.ts`, `src/agent/subagent.ts`) — from a 30-cycle overnight HashPet run (`7e705c79`, 29 commits, ~$132, ended on the provider's spending limit). Every harness fact was green thirty times and the product stopped improving after cycle 2: cycles 3–20 were "X sits with Y" first-hour copy slices, cycles 21–29 one idea — "digested, not ate" — one string per cycle, each Planner opening *"Cycle N taught …; Y still says …"* and each Out-of-scope list naming the next six renames; every Reviewer shipped. Not one plan came from using the extension. Four changes, none a meter: (1) **the Planner runs the product before it reads source** — the role now has full `bash` + MCP (browser) with the file-editing tools removed (`denyEdits`; read-only plan mode denied `npm run build` and `node dist/cli.js`, so a Planner that could only grep planned from grep) and writes what it did under a new `Looked:` line; (2) **the brief is a record, not a thread** — one line per shipped cycle (title · direction · verdict · worth · commit) plus the last review's Must-fix and shape notes; the previous plan's body and Out-of-scope list are no longer handed on; (3) **the Reviewer judges the cycle, not only the diff** — a new `Worth: yes|no — <why>` line answers "would a user notice this?", a `no` reaches the next Planner, the Reviewer's brief carries the last four cycles' `Worth:` and the third invisible cycle in a row is a `Must-fix`; the Reviewer also owns persisted data and public surface (a re-keyed store or changed API needs a migration or a named break); (4) **"leave it" is a plan** — with no mandate, `Verdict: fulfilled — the product is in good shape` ends the run; the rank for every candidate is "would a user notice this in their first minute, first day"; a vocabulary or consistency gap is one item across every surface or out of scope, never one string per cycle. Also: the guideline audit resolves `cd <dir> && npm test` against `<dir>/package.json` (HashPet's `CLAUDE.md` was flagged stale for every command it documented correctly); `Operator: omit` parses as empty. Job: a run that has fixed what a user would notice stops instead of renaming strings until the credit card does.
- **ULW gate: baseline, order, isolates, blocked** (`src/harness/cycle/verify.ts`, `orchestrator.ts`, `verification.ts`) — from the first two dogfood runs of the plan-cycle driver on this repository. (1) **The gate is judged against a baseline.** Cycle 1's plan admission runs the verify command once on the untouched tree and records its failing tests (`verify.baseline.log`); after every commit the accepted run becomes the baseline. Green is exit 0 **or** "every failure was already failing"; new failures are red and the FIX re-anchor names them (`New failures (yours to fix): …`) and says how many pre-existing ones do not count — a clone with no `dist/` and no TTY had 37 environment failures, and the executor spent three fix rounds editing colour tests it had never broken. A red run that names no failing test (crash, timeout, unknown runner) is red whatever the baseline says; `extractFailingTests` knows node:test / TAP / jest / vitest / pytest / cargo / go / mocha. (2) **Verify runs before the Reviewer**: the close sequence is verify → (fix) → review → verify → (fix) → commit, so the Reviewer reads a tree that passes and everything the executor changed to get green is reviewed; the second run covers the Reviewer's own revisions (cycle 1 of run 2 committed 21 files, seven of them tests edited after the review). (3) **An isolate is not the gate**: a `Verify:` line naming one test file or a typecheck is proof=ran; the stack table's suite gates and the isolate stays a declared check (run 1 declared `npx tsx --test tests/cycle-status.test.ts` and the suite never ran before the commit). (4) **Reviewer `blocked`, or a review that does not parse, closes the cycle with no commit** — the work stays in the tree and the next Planner starts from the `Must-fix` (the dogfood Planner found this gap on its own and planned it as cycle 1). Also: the plan admission lists the todo ids so the executor updates them instead of adding copies; the harness's own check feeds the run-level `verification` totals (headless JSON said `passed: false` after a green cycle because only the executor's isolates had been counted); `commitDirtyTree`'s subtree scope compares real paths (macOS `/tmp` → `/private/tmp` made the guard read the workspace as outside the repo and skip). Job: a real repository with a red baseline can be worked in, and nothing reaches the commit unreviewed.
- **ULW is a plan-cycle driver; the wave engine is gone** (`src/harness/cycle/`, replacing `ulw-cycle.ts` and the twelve wave-meter modules — `same-surface`, `idea-surface`, `tree-shape`, `bet-contract`, `explore-contract`, `job-delta`, `tests-without-body`, `last-reflect`, `ulw-job-card`, `work-class`, `product-quality`, `ulw-plan-mode` — with their tests). The unit of work is a **cycle**: PLAN → EXECUTE → REVIEW → VERIFY → COMMIT → re-plan. A **fresh-context Planner** subagent (empty transcript, its own `[ulw] planner_model` / `planner_effort`) reads the product's identity, researches its category online, surveys the whole tree and writes `cycles/<n>/plan.md` (`Verdict:` / `Identity:` / `Direction:` / `Verify:` / `Items:` / `Out of scope:` / `Guidelines:` / `Operator:`); the session model **executes** the items as its todo board for as many waves as the plan needs and closes with `Plan complete.`; a **fresh-context Reviewer** subagent reads the plan and `git diff <cycle start>` as a reviewer and as an architect, **revises in place**, and writes `review.md` (`Fulfillment` / `Revisions` / `Must-fix` / `Architecture`); the **harness itself runs the verify command** and commits on green (`ulw cycle N: <title>`, local only) — a red check sends the executor back with the tail for `fix_rounds` (3), then releases with nothing committed; the next Planner reads the previous plans, reviews, must-fix, unfinished items and anything the user typed. Job: the three prompts a user actually gives — a clear goal (`/ulw add --version with a test`: cycle 1 ships it, the Planner writes `Verdict: fulfilled`, the run stops), a direction (`/ulw polish the first run`), and nothing (`/ulw`: the Planner derives the direction from the product) — are one procedure that differs only in where the direction comes from, and a mandate that is fulfilled ends the run instead of metastasising into unasked work. **There is no mandate classifier** (`isSoftPrompt` / `isOpenMandate` decided from regex over `improve|polish|fix` and a `/` in "UI/UX" which contract governed a 250-wave run; "fix the flaky tests" owed a Bet, "improve the UI/UX comprehensively" did not) **and no meter**: HashPet (`23b2c2a5`, 791 waves) satisfied every stalling meter and three accretion meters were then added — a fresh Reviewer reading the cycle diff once per cycle sees in one look what the meters approximated one wave at a time. The harness enforces sequence and facts only: no writes before a plan artifact, no commit before a completed review and a green harness-run check, no Stop mid-cycle; `stuck_threshold` (4) no-progress Stops route to the Reviewer, never to a release. Controls: `/cycle 0` = finish the open cycle (execute → review → verify → commit) then stop; `/cycle 1`; **`/replan`** (new) closes the cycle now; **`/max-cycles N|off`** replaces `/max-waves` (kept as an alias that prints the rename); `/plan` is a human pause of the harness Planner, `/build` hands it back; `enter_plan_mode` from the executor requests a re-plan instead of pausing writes. `--max-cycles` on `forge` / `forge run` (`--max-waves` aliased); headless JSON drops `ulwWave` / `ulwMaxWaves` / `ulwSoftPrompt` / `ulwExpandedMandate` and carries `ulwPhase`, `ulwMaxCycles`, `ulwCycles[]` (per cycle: items, review verdict, verify, commit), `ulwEndReason`, `verification`. `ulw.json` is schema 2 (a schema-1 sidecar loads as `legacy`, disabled — `/ulw` re-arms); `cycles/<n>/plan.md` / `review.md` / `verify.log` are the run's record; the run report lists cycles, per-cycle verify results, open items and `Must-fix`; `lastError.code` is `ulw_done` for a designed end (fulfilled / `/cycle 0` / `max_cycles`) and `ulw_released` otherwise; metrics gain `ulwEndReason` / `ulwCycles` and `forge stats` prints `fulfilled=` / `cycles=`. `[ulw]` config table: `planner_model`, `planner_effort`, `reviewer_model`, `reviewer_effort`, `max_cycles`, `fix_rounds`, `stuck_threshold`; env `FORGE_ULW_VERIFY_TIMEOUT_MS`, `FORGE_ULW_PLANNER_MAX_TURNS`, `FORGE_ULW_REVIEWER_MAX_TURNS`. Subagents gain `role` / `model` / `reasoningEffort` / `inlineSkills` on the request and `verification` on the result; the Planner may spawn explore children, the Reviewer writes on the live tree. Skills: `forge-planner` and `forge-reviewer` (new, inlined into the role briefs), `forge-veteran` rewritten as shared doctrine. The system prompt's ULW block is a short **executor protocol**; the report guard's `**Cycle complete.**` attestation pass is gone (the driver releases on its own facts and prints the report). Verification classification moved unchanged to `src/harness/verification.ts`; `git-auto-commit.ts` is a generic `commitDirtyTree` with the same staging rules (secrets, fixtures, `.forge/` scratch, unreferenced looks) plus one new one: **a workspace inside a repo commits only its own subtree** — `git rev-parse` walks up from a monorepo package or a bare temp dir, and the cycle commit used to sweep the enclosing repo's dirty files (a test fixture once committed this repository's working tree as `ulw cycle 1: test plan`). The pin-budget law (`util/pin-budget.ts`) is retired — test hygiene is the Reviewer's duty. `docs/ULW.md` rewritten.

### Added
- **The harness measures accretion, not only stalling** (HashPet dogfood, session `23b2c2a5`: 791 waves, 599 commits, ~27 h, ≈$740). Grok scored that run's product 55 → 93 and its architecture 74 → 36 — `resolveHomeIntent` grew from 4 to 15 arguments, 440 `export function`s landed in the same ~40 files, `sitDays >= 2` appeared in 114 places, 319 comments said "used to", a hard-`false` flag kept eight assertions alive, 78 unreferenced look PNG/HTML were committed — while every ULW meter stayed green: proof ✓ on 256/256 retained waves, job-move 75%, `onBet` 52%, thin streak 0, same-surface streak never above 1. Every rule measured whether the run was *moving*; none measured what it was *growing*. Five structural rules now do, each a pure `evaluate…` with an env kill-switch, wired in `ulw-cycle.ts`, metered under `[Forge ULW cycle driver]`:
  - **A Bet is a capability with a new path** (`src/harness/bet-contract.ts`): 234 `Bet:` lines were adopted in that run, 96 shaped like holes ("MOOD still hunts while STATUS says ATE — `pet-face.ts`"), none naming a file that did not exist — and a hole with a path was a slice, a slice was a job move exempt from the same-surface hold, and 71 swaps cost nothing. `parseBetLine` now returns `kind: "hole"` when the capability clause (before the first dash) carries defect grammar (`still`, `instead of`, `no longer`, `never`, `does not`, `wrong`, `leaks`, `ignores`, `fails to`, `broken`, `crash`, `regression`, `bug`, `fix`, …; `cannot` stays — it is the grammar of a gap); `maybeAdoptBet` resolves the bet's paths against the tree and refuses one whose every path already exists. Adopted bets carry `newPaths`, and a **slice is a production change on a path the bet creates** (or inside a new directory) — `betPathHit` no longer credits every edit in `extension/src/lib/` because the bet named one file there. The refusal rides the re-anchor (`⚠ Bet refused — "still" describes a defect…`) and clears on the next real bet. Past `BET_RUN_ADOPT_SOFT` (6) bets in a run, replacing a bet with fewer than `BET_STUB_SLICES` (2) slices is a swap; status shows `7 bets this run (1 swapped unshipped)`. Without a cwd (tests, memory seeding) the new-path test is skipped and the old file-or-directory rule applies.
  - **Idea-surface hold** (`src/harness/idea-surface.ts`, new): the same-surface hold is per *file*; the paint was per *idea* — "sit-days hushes the lecture" shipped in 114 waves across 33 files, "names the found chew" in 84 across 29, ten in a row, each a different file, each the same sentence, and 55–60% of them on-bet, which reset the per-file streak to 1 by construction. The new hold keys on the closer's distinctive terms and bigrams (backticked identifiers whole; ritual words, filler adverbs and run mantras — terms in ≥60% of the window — dropped) against the last `IDEA_LOOKBACK` (10) credited ships. One idea on `IDEA_FILE_HOLD` (4) distinct production files **with no file in common** holds unlimited ULW — a paint has no home, a build-out touches its module on every slice. On-bet and on-contract ships are **not** exempt. Release: a collapse (a new module — diff kind, or a path the ledger never saw — touched with a carrier, or three carriers rewired at once; the comparison window restarts there), a different idea, or `/cycle 0`. Advisory on the re-anchor at 3 files; `/cycle status` `Idea surface:` line; LAST-reflect Must-fix when the paint was never collapsed. `FORGE_ULW_IDEA_HOLD=0`.
  - **Tree-shape meter** (`src/harness/tree-shape.ts`, new): at every consolidation wave the run's cumulative diff (`git diff <HEAD-at-arm>` plus untracked production files; `startHead` is recorded at arm) is read for facts — exports added to existing files vs new modules, added signatures with 7+ parameters (single-line and prettier-wrapped, TS/JS/Python/Rust/Go), one predicate literal (`sitDays >= 2`) added in 4+ files (loop counters and length checks excluded), comment lines that narrate the change, `export const x = false`, new image/HTML files no tracked or untracked non-asset file references (`git grep -F <basename>`, capped). A **trip** is a fact past its threshold (`SHAPE_*` constants). Trips fill that consolidation's Must-fix, flip the consolidation prompt from "fix real defects only" to `TREE SHAPE tripped … this consolidation is a COLLAPSE`, ride `/cycle status` (`Tree shape: ⚠ 1 trip(s) at w4 — +12 exports (…) · \`sitDays >= 2\` in 5 files`), and — when the same trip is present and no smaller at the next consolidation — **hold** Stop until a ship makes that number go down (re-measured at the gate; the first decrease releases) or `/cycle 0`. Pinned by a real-repo test: predicate forks trip at w4, hold at w8, a 9th fork is refused, the `isSitting()` collapse releases. `FORGE_TREE_SHAPE=0` (meter off), `FORGE_TREE_SHAPE_HOLD=0` (hold off).
  - **Capability drought** (`ulw-cycle.ts`): `capabilityDrought` counts credited ships since the last capability ship — a new production module (diff kind, or a path the ledger never recorded) or a bet slice on a path the bet creates; `capabilityShips`, `jobMoves`, `creditedShips` are run-wide counters the 256-row ledger cannot answer. Advisory on the re-anchor at `CAPABILITY_DROUGHT_ADVISORY` (12); an open, unlimited, undeclined mandate **holds** at `CAPABILITY_DROUGHT_HOLD` (24) — "an open mandate that only repairs is a bug tracker, not a product" — until a new module or new-path slice lands, `Bet: none — why`, or `/cycle 0` (the count stays; it is a run fact). Hard mandates and capped runs only advise. `/cycle status` `Capability:` line; LAST-reflect Must-fix at the bar. `FORGE_ULW_CAPABILITY_HOLD=0`.
  - **Spend against what it bought** (`formatUlwSpendLine`): `/cycle status` prints `Spend: $40 this run · $2.00 per wave · $4.00 per job move (10) · $20 per new module (2)` from the session's tokens at the provider's rates; silent when unpriced (Cursor quota).
- **Auto-commit leaves looks and scratch unstaged** (`src/util/git-auto-commit.ts`): the play-loop asks for a screenshot, the model wrote it into `images/`, and `maybeAutoCommitOnUlwDone` staged everything dirty — 78 PNG/HTML (968 KB) in HashPet's repo that the extension never loads, plus 19 `.forge/chrome-look*` Chromium profiles (36 MB) in the project's `.forge/`. A **look** (image/HTML whose name or directory says screenshot/look/shot/capture/preview/states/before/after) that no tracked or untracked non-asset file references is left unstaged; a sprite the manifest names commits as usual. `.forge/` is staged only for `MEMORY.md`, `AGENTS.md`, `hooks.json`, `config.toml`, `commands/`, `skills/`, `hooks/`. The result carries `leftUnstaged`; the loop posts one `[Forge harness — mid-conversation update]` naming the files and where they belong (`~/.forge/sessions/<id>/looks`, `sessionLooksDir`), and the run's final text gets a `Left unstaged (…)` line. A dirty tree of looks alone is not a commit (`only look artefacts / scratch remain`).

### Changed
- **System prompt**: two operating principles — screenshots, look HTML and browser profiles go under `~/.forge/sessions/<id>/looks`, never into the repo; comments describe the code as it is, never the change ("used to", "no longer", "previously") — HashPet's `lib/` carried 319 such comments, a diary of the agent, not an API. The ULW doctrine block folds in the new rules (consolidation reads the diff as an architect and names the six shape facts; the idea hold; a Bet names the **new file** it creates and a defect in Bet grammar is refused; 24 ships with no new module hold an open mandate) and loses its duplicated `/cycle 0` sentence and the "Other force multipliers" section that repeated the operating principles. `forge-veteran` carries the same three lines (Bet grammar with the file it creates, looks outside the repo, code written as if it had always been that way; "one idea on a third file with no shared module" is a stale-plan signal).
- **The consolidation prompt asks for a collapse, not only defects**: "Fix real defects only" told 59 consecutive HashPet consolidations that duplication, arity and dead flags were out of scope; the wave now reads the cumulative diff "as a hostile reviewer — and as an architect", and when the tree-shape meter tripped, the collapse is the wave's job and counts as a job move.
- **Auto-commit subjects drop the model's own `Wave N —` prefix**: HashPet's subjects said `Wave 160 — Consolidation` while the harness body said `Wave 791.` — the model's count had drifted 5×. The body carries the harness wave; the subject carries the ship.
- **Provider rounds are priced** (`src/session/prompt-cache.ts`): `rounds.jsonl` rows carry `estCostUsd` (the provider's rates, cached input at the cache rate), so the price of a wave is readable from the round ledger alone — the 7,970-round HashPet run summed to $0 there because only `run_end` was priced and the REPL's `run_end` carried no tokens.
- **`tests/harness-pe.test.ts` prompt ceiling measures the code, not the developer's memory**: the fixture's empty `.git` dir was not a repo, `git rev-parse` walked up to this repo's root, and `## Project memory` (1.7k on this machine) rode into a test about doctrine size. The fixture is `git init`ed (the `AGENTS.md` fixture rule), the test asserts no project memory is present, and the ceiling is ratcheted from 17k to **16k** — the honest measurement is 15.6k.

### Added
- **Rule files are never silently clipped** (`src/agent/system-prompt.ts`, `src/agent/instruction-paths.ts`): the prompt loader cut every rule file at a flat 12k and said nothing — this repo's own `AGENTS.md` lost its Conventions and Non-negotiables for weeks. The 28k total is now split fairly across the files actually loaded (`ruleFileBudget`: 12k floor, more when there are fewer files, so a lone 27k `AGENTS.md` loads whole), a clipped file ends with a visible `[clipped: N of M chars — headings not shown: …]` marker the model can act on, the REPL startup posture warns `AGENTS.md clipped (12.0k of 26.8k)`, `/context` prints `loaded/total` per file with the unseen headings, and `forge doctor` gets a `rulesClipped` row / JSON field.
- **Guideline audit fixes facts and proposes doctrine** (`src/harness/guideline-audit.ts`, rewritten): the first version told the model its rewrite of the user's `AGENTS.md` was "authorised whatever the file says" and blocked a Stop until it read the files. Two kinds of wrong now get two treatments. **Fact defects** — `stale-paths`, `stale-commands` (a `package.json` script / Makefile target that is not there), `pm-mismatch` (only against a lone lockfile or an explicit `packageManager` field), `clipped`, `empty` — are fixed in the file by the model; nobody wants a dead path in their map. **Doctrine** — `long`, `conflict`, `no-commands` — is written to a proposal outside the repo (`~/.forge/guidelines/<project>/<file>.proposed.md`) and the tracked file waits for `/guidelines apply`; `/guidelines diff` shows it, `/guidelines discard` drops it, `guideline_auto_apply = true` (`[guidelines] auto_apply` · `FORGE_GUIDELINE_AUTO_APPLY=1`) lands it directly, journaled for `/undo`. The trigger is evidence, not a 14-day calendar: a fact defect, or a body change since the last proofread plus any issue. A stamp means "no fact defects at this hash", doctrine never withholds it, `/guidelines stamp` acknowledges the current issues until the body changes. **The Stop block is gone** (`FORGE_GUIDELINE_AUDIT_BLOCK` no longer read): an ignored fact brief is a line in the run report and re-briefs next session.
- **Prose classifiers are pinned to a corpus** (`tests/fixtures/prose-corpus.ts`, `tests/prose-classifiers.test.ts`): three regex classifiers judge natural language — is the user asking or ordering (`advisory-intent.ts`), is the closer handing homework back, is it shaped like a run report (`report-guard.ts`) — and each had been tuned against the sentence that last broke it. The corpus holds labelled user messages, closers and reports with the reason for each label; every classifier change has to keep it green, and the fixture is where a new false positive goes first.
- **Harness overhead is a number** (`src/session/metrics.ts`, `src/session/request-prune.ts`): `forge run --json`, `metrics.jsonl`, and last-run session meta carry `guardBlocks` — Stop-guard bounces by guard (`handoff`, `proofClaim`, `report`, `todoGate`, `goal`, `ulw`, `hook` for a user Stop hook, `verify`, `fix`, `doomLoop`, `errorStreak`, …) — and `forge stats` / `/stats` print a `harness:` row (pokes as a share of provider rounds, proof pokes, blocks per guard, how many runs were metered). An over-eager guard costs rounds you can now see and alert on.
- **`docs/MODULES.md`**: the per-module doctrine that was the body of `AGENTS.md` before it was cut to a map (44 module contracts + the production-reliability catalog), restored from history and grouped by directory, with the report-guard / guideline-audit / advisory-intent bullets updated to this release. `docs/HARNESS.md` describes the fact/doctrine audit and the narrowed report guard; the dead step-1c paragraph is replaced by why there is no guard.
- **Agent-guidelines audit is the first action of a session** (`src/harness/guideline-audit.ts`): a badly written `AGENTS.md` / `CLAUDE.md` caps every session no matter how strong the model is — this repo's own `AGENTS.md` had grown to 26.8k chars while the prompt loader clips each file at 12k, so no agent ever saw its Conventions or Non-negotiables. The harness now surveys the guideline files (freshness by stamp + hash, `manual` over the cap, `no-commands`, `stale-paths`, anti-verification `conflict` rules, import-only pointers, missing primary), briefs the model as the first harness message with best practice and the statement that activating Forge authorises revising or rewriting them whatever they say, records whether the model actually looked, stamps `<!-- proofread <UTC> · forge -->` **only when the re-survey comes back clean** (a revision that trims a 13k file to 12.4k is still over the cap, so it is reported as `AGENTS.md revised by the agent (…) but not stamped — over the 12,000-char cap …` and audited again next session — a stamp would tell the next fortnight of sessions the file is fine; sibling harness stamps count), keeps hashes in `~/.forge/guidelines/<projectKey>.json`, and tells the user what really changed (`AGENTS.md revised by the agent (139 → 68 lines, 26.8k → 6.6k chars)`). Deferred while plan mode / ULW orient deny writes; subagents never audit; a briefed-and-ignored file blocks Stop once. `/guidelines [audit|stamp]`, `forge doctor` row + `guidelines` / `guidelinesAuditDue` JSON, `forge run --json` `guidelines`. `FORGE_GUIDELINE_AUDIT=0` off, `FORGE_GUIDELINE_RECHECK_DAYS` (14). Job: open any repo, the first thing the agent does is fix the map it will steer by.
- **Standalone run report** (`src/harness/run-report.ts`): the end-of-run and status views stand on their own — the user will not scroll, and after LAST reflect the model's last message covered only the close-out. Outcome sentence first (`Done — 3 files changed, verified with \`npm test\``, `Paused — /cycle 0 sat down after 12 waves; ULW stays on`, `Winding down — ULW is on its last cycle … the wrap, LAST reflect and **Cycle complete.** are still ahead`, `Partly done — … the last check is RED`), then **What shipped** (waves since the mandate, commits landed in the run window, files changed) · **Verified** · **Not done** (open todos, open named ships, unsliced bet, LAST must-fix, goal criteria, cap/stuck reasons) · **Agent guidelines** · **Needs you** (`Operator:` items only, else Nothing) · **Resume**. **One report per run, not two**: at the end of a model run the closer *is* the report (the guard shapes it, on the terminal attestation too), so the harness prints only what the model could not know — the guideline audit, resume, the saved path — and falls back to the full card when the closer is not report-shaped or when no guard read it at all (stuck-wall, spend cap, turn cap, continue cap — the endings whose reason lives in **Not done**). Printed after multi-round runs and every driver end in the REPL and headless, in full on `/done` (a typed command has no closer behind it) and `/report` any time, the outcome + open items head `/status`, `forge run --json` → `report`, saved to `~/.forge/sessions/<id>/report.md`. Ledger prose is stripped of the markdown it was written in (real ledgers hold `** CLI \`together run\` still cannot…`) and the commit count is the real total, not the 20-row display page. The LAST wrap card and the system prompt ask the model for the same run-wide shape.
- **Report guard** (`src/harness/report-guard.ts`, stop-guard step 8 **and step 1b**): the closing message may not hand homework back — "you can now run…", "next step for you: add…", "you'll need to configure…" is bounced once with the harness facts; the only things left to the user are a missing secret, a hard external blocker, or an irreversible action, each as an `Operator:` line (lines naming those reasons are exempt). After ≥ 2 harness rounds with edits, a last-round-only closer ("Addressed the nit.") is bounced once for the run-wide shape. The closer this tool actually produces is a **driver attestation**, and the goal / ULW drivers release on it and return before step 8 can look — so the same two checks also run as **step 1b, ahead of every driver**, on `**Cycle complete.**` under `/cycle 0` and on `**Goal achieved.**` (a cycle=1 `**Cycle complete.**` declares a wave and is left alone). Blocking there spends no wave, no evidence nudge and no wrap flag: the model re-attests and the driver releases as it would have. Advisory Q&A never bounces. `FORGE_REPORT_BLOCK_CAP` (2), `FORGE_REPORT_GUARD=0` off.

### Changed
- **Report guard flags directives, not affordances** (`src/harness/report-guard.ts`): "you can now run `npm test`" told the user what the result lets them do and was bounced as homework, so the model learned to say nothing about what it built. Homework is now a directive — "you **should** run", "you'll **need to** configure", "next step for you:", "when you're ready, run", "run X **and fix** Y" — while `can` / `could` / `may` pass; the allowed-reason list adds environment blockers and user decisions. The report shape accepts any markdown heading or a short standalone bold/underlined label as a section, not only the four words the old regex knew; the re-anchor gives the four as examples. All of it against the corpus.
- **"Final report" in the system prompt is proportionate** (`src/agent/system-prompt.ts` → "Closing message"): the section demanded a four-section report of every closer, so one-round fixes and answers to questions grew headings. A question gets an answer; a one-round fix a sentence and the check that ran; a multi-round run gets the whole-run report, headings of the model's choosing. "Telling the user what they can now do is fine" is stated so the guard and the prompt agree. Prompt stays under the 17k cache-stable ceiling.
- **`provider_round` events moved to `~/.forge/rounds.jsonl`** (`src/session/metrics.ts`): one ULW run writes hundreds of rounds, and while rounds and `run_end` shared a 2,000-line `metrics.jsonl` the auto-prune evicted every run record within a day — `forge stats` was reporting on the last few hours. Runs stay in `metrics.jsonl` (prune 2,000), rounds get their own file (prune 5,000), the per-session `rounds.jsonl` sidecar still gets both, `forge prune-metrics` cuts both, `/metrics` shows both counts.
- **`AGENTS.md` cites the new facts**: fair rules budget + clip marker, stop-guard order without the removed step 1c, the fact/doctrine audit, `metrics.jsonl` vs `rounds.jsonl`, the `[Forge <guard>]` prefix + `GUARD_BLOCK_CLASSES` rule for new guards, the corpus rule for regex classifiers, and `docs/MODULES.md` as where deep module detail lives. Still 8.2k — under the budget it describes.
- **`npm test` clears its own scratch**: the script pins `TMPDIR` inside the repo and fixtures leave their temp dirs behind, so a `.tmp` grown to 156,000 files (a day of repeated runs) made `tests/ulw-verification-credit.test.ts` time out at 10s on a background task spawn — a failure that looks like a product bug and is not. `rm -rf .tmp/forge-*` runs first; logs parked in `.tmp` are untouched.
- **`/guidelines stamp` and `/guidelines audit` wait for idle**: the status card is safe mid-run, but stamping writes the repo's `AGENTS.md` while the agent may be editing it, so `classifyLiveSlash` sends both verbs to `idle-only` (the `/budget` / `/hooks` pattern).
- **`AGENTS.md` is a map again**: rewritten from 26,795 chars / 139 lines of feature catalog to a stamped 6,585-char / 68-line map (commands, layout, conventions, non-negotiables, working here); the catalog detail already lives in `CHANGELOG.md` / `docs/`.

### Fixed
- **Interactive runs record their harness meters** (`src/tui/repl.ts`): the REPL's `run_end` never passed `harnessUserPokes` / `admitCount` / `proofPokes` / `providerRounds` — only headless did — so every dogfood session on this machine read as poke-free and the cost of the harness was unmeasurable in exactly the runs that mattered. Passed now, with `guardBlocks`.
- **A user Stop hook's block is filed under `hook`** (`src/agent/loop.ts`): a hook reason carries no `[Forge` tag and the loop re-tags it `[Forge ULW cycle driver]`, which would have counted it as a ULW bounce; the loop now appends `[Stop hook]` so the meter files it correctly (request-prune's stubbing class is unchanged).
- **`npm install` is never a stale command** (`src/harness/guideline-audit.ts`): the stale-script check treated `install` / `ci` / `publish` / `version` as script aliases, and this repo's own `AGENTS.md` was flagged for its `npm install` line on the audit's first real run. Only `test` / `start` / `stop` / `restart` need a script.
- **`npm test` runs on a grown `.tmp`** (`package.json`): `rm -rf .tmp/forge-*` fails with `Argument list too long` once the glob passes `ARG_MAX` — the state the script exists to clear. `find … -exec rm -rf {} +` now.
- **A question is never audited, bounced or stamped** (`src/harness/guideline-audit.ts`): the audit fired on any first prompt, so asking `what does this repo do?` in a repo with an unstamped `AGENTS.md` cost an injected proofread brief ahead of the answer, a blocked Stop demanding the files be read and revised, and a real `<!-- proofread <UTC> · forge -->` write into the user's tracked file — and under ULW `finalizeGuidelineAudit` runs immediately before the release auto-commit, so that write landed in a commit nobody asked for, off the back of a question. Every other Stop-blocking guard in `src/harness/` already carves Q&A out with `looksLikeAdvisoryUserMessage` (report-guard, todo-gate, handoff-guard, proof-claim-guard); the audit now does it at all three of its turn-acting doors — brief, Stop, stamp. Reading `AGENTS.md` to *answer* a question is not a proofread, and `noteGuidelineToolCall` only records while a brief is open, so no evidence accrues either. It is a **defer, not a skip**: `phase` stays `pending`, the one-block cap stays unspent, and the next work prompt of the same session audits exactly as it would have. The brief and stamp doors read `lastRealUserPrompt(session)` rather than the last user-role row, because by the time a safe boundary comes round that row is a harness admit. Job: ask a question, get an answer, and find your repo unchanged.
- **The audit belongs to the turn that ran it**: `finalizeGuidelineAudit` handed the stored result back unflagged on every later call, so each subsequent prompt of the same session re-printed `AGENTS.md proofread … stamp updated` and hung `guidelines` on a run that had audited nothing. The second call is flagged `repeat` now, and every run-scoped surface reading that stored result honours it: the loop's own notice, the `guidelines` key on the run result, and the run report's **Agent guidelines** section, which falls back to the plain survey line so a later prompt's report says what the files are like now. The `/guidelines` card is deliberately not gated, because it is session-scoped and prints under its own `this session:` heading.
- **A proofread stamp has to be earned** (`src/harness/guideline-audit.ts`): the audit credited a look on any tool call whose arguments merely *contained* the file's rel path — and for a root-level primary the rel **is** the bare basename `AGENTS.md`, so one `grep { pattern: "AGENTS.md" }` (or a commit message, or a diff quoting the name) both wrote a real `<!-- proofread <UTC> · forge -->` stamp into the user's repo and released the step-1c Stop block, for a file that was never opened. A stamp silences the audit for `FORGE_GUIDELINE_RECHECK_DAYS`, so a false one costs a fortnight of sessions, and the module's own comment claimed the opposite ("the stamp is an attestation; it has to be earned"). A file is credited now only when a **path-carrying argument resolves to it**: path arguments (`path` / `file_path` / an `apply_patch` `*** Update File:` header …) are resolved against the workspace, then the survey root, and compared to the file's absolute path — a regex, a glob, a commit message and file *content* resolve to nothing. `grep` and `glob` are no longer a look at all, and neither is bash `grep` / `rg`: a pattern match returns matching lines or matching paths, not the file, and the brief asks the model to read each flagged file and judge it. bash is segment-strict like the rest of the shell handling here, so `cat docs/notes.md | grep AGENTS.md` credits nothing while `cat AGENTS.md`, `sed -n '1,40p' AGENTS.md`, a write redirect and `sed -i` still do. A body-hash change still credits a look through any channel. Job: `grep` for a symbol that happens to sit in `AGENTS.md` and the audit still asks you to read it.
- **The audited guideline file is the one the prompt loaded** (`src/harness/guideline-audit.ts`, new leaf `src/agent/instruction-paths.ts`): `loadProjectRules` walks workspace → git root and lets a nested `AGENTS.md` shadow the monorepo root, while `surveyGuidelines` read a fixed list at the git root only. Run forge in `repo/packages/api` with a local `packages/api/AGENTS.md` saying "never run the tests" and the prompt was steered by that file while the audit proofread `repo/AGENTS.md` and reported it fine — the conflict rule never fired, and the feature that exists to audit the map you steer by audited a different one. The walk now lives in one leaf module both sides call, so the audited set is the loaded set; `rel` stays relative to the resolved root (`packages/api/AGENTS.md`) so registry keys and every display string still read the same. The `.cursor/rules` scan follows the walk too, `resolveGuidelineRoot` is the loader's own ceiling, and `GUIDELINE_MANUAL_CHARS` is the loader's `12,000` constant instead of a copy that could drift from the cap it names. Two differences from the loader are deliberate, named in the module header, `docs/HARNESS.md` and a test that pins the name sets: the audit surveys sibling-tool maps the prompt never loads (`AUDIT_ONLY_GUIDELINE_FILES` — `GEMINI.md`, `.windsurfrules`, `.clinerules`, `.claude/CLAUDE.md`), and it never *surveys* `~/.forge/AGENTS.md`, the loader's global fallback, because that is the user's file rather than this project's map — a project with none of its own is still `missingPrimary`. It is **named** now, though: reporting `AGENTS.md missing` while `~/.forge/AGENTS.md` sat in the prompt steering every turn told the user about a hole and not about the file doing the steering. `surveyGuidelines` carries `globalFallback`, the doctor / `/status` line reads `AGENTS.md missing · ~/.forge/AGENTS.md steers instead`, and the brief says it is in force and must not be edited. The old test compared one filtered basename in one fixture; the set relationship is pinned now in both directions, at the name level and over a fixture holding every file in both lists.
- **One guideline registry per repo, not one per spelling of its path**: `~/.forge/guidelines/<key>.json` hashed the raw walk result while project memory hands `projectMemoryKey` a `git rev-parse` root, so the same repo reached through a symlink (or as `/var/…` rather than `/private/var/…`) kept two registries — and the second had no hashes, so `edited since proofread` could not be detected there. The key is `realpath`ed now; the survey root stays the literal walk result, because it is the ceiling of the prompt loader's walk and every `rel` is relative to it.
- **The guideline-audit block is reachable in a ULW run** (`src/harness/stop-guard.ts`): the guard shipped at step 9, behind the drivers, and `evaluateUlwAtStop` answers a Stop neutrally **only when ULW is off** — while it is armed every path either blocks or sets a release flag (`stuckReleased` / `lastCycleReleased` / `lastCycleSatDown`) and stop-guard returns on both. So in every `/ulw` session the guard never ran: a model that ignored the "proofread `AGENTS.md` first" brief was never bounced, and a repo whose `AGENTS.md` says "never run tests" kept steering all 250 waves — the one run where the map matters most. It now sits at **step 1c, beside the report guard's attestation pass and ahead of every driver**, so the bounce spends no wave, no evidence nudge and no wrap flag; still capped at one block per session, still `FORGE_GUIDELINE_AUDIT_BLOCK=0` to turn off. Pinned by a loop test that arms ULW and counts the bounce.
- **`NO_COLOR` reaches the run report** (`src/harness/run-report.ts`): `renderRunReportText` and `renderRunReportAddendum` hand-roll `\x1b[1m` / `\x1b[2m` rather than going through chalk, and chalk is the thing that self-disables under `NO_COLOR` (`src/util/log.ts` forces `FORCE_COLOR=0`), so `/report`, `/done`, the REPL end-of-run print and headless `forge run` all painted escape codes into a piped log while every neighbouring surface (`/status`, the bottom dock, the statusline) went plain. All four callers pass a bare `Boolean(process.stdout.isTTY)`, so the veto now sits **inside** the two renderers, where a fifth caller cannot skip it. Swept the class and found two more, both wider: the streamed markdown renderer and the `forge ›` / `think ›` turn openers build a Chalk at `Math.max(chalk.level, 1)` because chalk under-detects some real TTYs, and that max undid `NO_COLOR` on **every assistant message in the REPL** (`FORCE_COLOR=0` drops chalk to 0, the max puts it back to 1); they go through `forcedChalkLevel()` now, which returns 0 under `NO_COLOR` and keeps the renderer's widths and tables. And `clipAnsi` grafted a trailing `\x1b[0m` onto text that had no colour in it at all (a clipped session title, a `/help` row); it closes only what was opened now, like `closeAnsi` beside it.
- **The run-report addendum lines up**: `needs you` was padded by hand to 11 characters while `guidelines`, `resume` and `saved` all reached 12, so the one row that carries an `Operator:` item — the only row asking the user for something — sat a character left of the column. The addendum pads to a single label field now, like `/status` does.
- **Background checks are proof**: the god-mode doctrine and every consolidation re-anchor say "never foreground the full suite — `background:true` then `get_task_output`", and `countsTowardVerification` then refused every background run, so the harness contradicted itself. Dogfood obeyed: a Swift menu-bar app ran 20 of 28 checks in the background and stamped proof=✗ on 248 of 256 waves; a Rust workspace ran 45 of 61 `cargo test`s in the background and stamped ✗ on 107 of 109; `fullSuitePassed` never flipped and LAST reflect demanded a hole no channel could close. The *spawn* still observes nothing, but a task that **settles** (`onBackgroundTaskSettled`) or is **joined** by `get_task_output` observes its exit code and log tail — `classifyVerificationRun` (ran / passed / isolate / full-suite) now credits it exactly like a foreground result, once per task (`creditedBgTaskIds`), into the wave ledger and the last-verify trail. A failed / timed-out / killed task is ran-not-passed (a hung suite stays proof=✗). Job: run `npm test` with `background: true`, `get_task_output` it, and the wave closes with proof ✓.
- **Checks in every stack are recognised**: `swift test` / `swift build`, `xcodebuild … test`, `zig build test`, `dotnet test`, `flutter|dart test`, `just|task test|ci`, `make verify`, `cargo nextest`, `bundle exec rspec`, `--self-test`, and repo scripts shaped like check runners (`./build.sh`, `scripts/test.sh`, `bash ci/check.sh`, `python tools/selftest.py`) are verification; whole-project suites (`cargo test --workspace`, `swift test`, `go test ./...`, `dotnet test`, `make check`) are full-suite; targeted runs (`cargo test -p x`, `swift test --filter`, `go test -run`) and compile-class checks (`cargo check|clippy`, `swift build`, `go vet`, `zig build`) are proof=ran. Quoted spans are ignored and observer/arranger heads never count — the dogfood trail had stamped `pgrep -lf 'cargo test --workspace' | head -3` as the last check. project-intel knows `Package.swift`, `build.zig`, `*.csproj`, `pubspec.yaml` and executable `./build.sh` / `./test.sh` / `scripts/check.sh`. New leaf module `src/harness/verify-command.ts`.
- **The Reading's verify command is adopted**: every Wave-1 Reading is asked for "the command that proves it" and dogfood wrote it (`./build.sh && --self-test`) — the harness threw it away. `src/harness/declared-checks.ts` harvests `Verify:` / `Proof:` / inline-code commands from Readings, Bets and closers onto `ulw.json.declaredChecks` (strict: an allow-listed runner head or a check-shaped script path; prose, `echo`/`pgrep`, and product verbs like `forge export --csv` are refused) and merges them ahead of the stack table for credit, full-suite credit and proof tips (`ulwPreferredCheckCommands`). Job: write `Verify: make selfcheck` once; running it is structural proof.
- **Proof demands re-arm**: `MAX_PROOF_DEMANDS` is per proof-less streak and only reset on proof, so a 256-wave run with no recognised check was asked twice — at waves 1 and 2 — then never again. Every consolidation wave resets the counter; a proof-less streak is challenged at least every 4 waves.
- **A look is a call, not a sentence**: `proof=play`, the product-quality look, and the chrome / pin / sibling-mill bypasses accepted "Play-loop:" / "played the game" / "zero JS errors" in the closer. A Swift menu-bar app with no browser stamped seven `play` waves by writing the word. All of them now require the structural `notePlayLoopRan` signal (a Playwright / browser MCP call or a screenshot `read_file` this wave); the look is consumed by the stamp.
- **Decision memory keeps what the run stands on**: the 400-record cap trimmed oldest-first, and both 400-record dogfood runs had lost their `MANDATE:` constraint (200 wave observations and 107 duplicate `Job:` rows did the evicting; the Wave-1 Reading was next). `trimDecisionRecords` evicts superseded rows, then waves beyond 48, then the oldest non-durable rows — priorities/constraints/blockers/out-of-scope, `MANDATE:`, `Bet:` and the first `Reading:` are never dropped. `Job:` / `Next need:` are one-slot notes: a new Reading supersedes the previous row (`supersedeMemoryRecords`) instead of appending, so compaction's memory budget is no longer spent on 100 copies of the same insight.
- **Job-move credit follows the open job**: `jobMoved` counted every explore path ever cited, forever — a single-file Swift app credited 185 of 256 waves as job moves because every edit touched `Sources/main.swift`, flattening bestWave anchoring, the off-job streak and "last job-moving ship". `collectUlwJobKeepPaths(…, { openOnly: true })` uses the Wave-1 reading's paths, open named ships, and explore picks not yet done (the prune keep-set stays broad). Reading path recognition covers Swift / Kotlin / Java / C# / C / C++ / Ruby / PHP / Elixir / Zig / Dart / Vue / Svelte too. The job card prints the **current** Reading beside Wave 1 after a re-PLAN.

### Changed
- **`Bet: none — why` is a window, not a waiver**: a decline quiets the bet contract for `BET_DECLINE_WINDOW`=6 credited ships, then the open mandate owes a `Bet:` again (`betRequired` returns, the off-streak restarts; the spent why is kept in `betDeclineHistory` and the same reason never declines twice). `/cycle status` and the job card show `Bet: declined (n/6 ships, then asked again)`; the re-anchor warns when two ships remain. The off-bet streak now counts **every credited ship** that is not a slice (a stamped wave that closes a hole is smoke whether or not the job-move classifier liked it) — previously only "job-moving" ships counted and that classifier was crediting 74% of dogfood waves. None of the 53 real runs had exercised the contract yet (all predate it); this makes the first one unable to opt out at wave 1 and grind 250 waves.

### Added
- **Open mandates owe a Bet (invent, not only repair)**: 45 of 53 dogfood ULW runs were soft/open (`improve the code`, "Improve the UI, UX, performance, reliability comprehensively", "be creative"); their 2,407 recorded waves were ~49% "X no longer Y" hole-closes by summary and the harness's own `editKind` classifier stamped **zero** `new-module` ships — every mechanism was satisfiable by a proven fix and the explore contract only produced holes (`pick:`). New `src/harness/bet-contract.ts`: an open mandate (`isOpenMandate` — soft, or improve-class with no concrete deliverable, length-independent) owes one `Bet:` in the Wave-1 Reading — a capability THIS product cannot do today that a demanding user would notice, with the path it lives in and the first provable slice. The kickoff, the PLAN prompt, `forge-veteran` and every CONTINUE re-anchor ask for it; there is no blocking demand (`Bet: none — <why>` declines once). Declared ships on the bet's files/directory are `onBet`: job moves that raise the bar and are never sibling mill (numbered `host-1/2/3.ts` inside the bet's directory are slices). Unlimited CONTINUE counts credited job-moving ships that touch no bet — on file or not — warns at 3 and **holds at 6** until a slice lands, a new `Bet:` with a path is written (two unshipped swaps, then only a slice), or `/cycle 0`; a restated bet does not reset; memory seeds only the first bet; capped runs and hard mandates never bet-hold. The bet rides the job card (re-anchor, compact, `/cycle status`) and decision memory; explore children may answer `bet:` beside `pick:` (candidates the demand reprints); `forge-veteran`, the PLAN prompt, the kickoff and god-mode expansion all carry the grammar. `/cycle 0` and a `/cycle 1` continue clear the hold like the same-surface hold (bet + swap count stay). Open wishes (`invent something valuable`, `build what's missing`) are soft prompts now; build orders with an object (`build a login page with email and password`, `design the onboarding flow`, `create a migration for the users table`) stay hard mandates with no Bet gate. Job: type `/ulw improve this tool`, get a capability shipped in slices — with the fixes as smoke, not as the run.

### Fixed
- **Project memory: a tracked mirror no longer gets shadowed by the local store**: `loadProjectMemory` returned the JSON store whenever one existed and read `.forge/MEMORY.md` only when it was missing — so a mirror that arrived from another machine, a moved path, or a `git checkout` of the tracked file could carry bullets the local store never had (this repo: 14 conventions/constraints, including the temp-index checkpoint constraint) and forge would never load them, then rewrite the mirror from the store on its next save. Mirror bullets absent from the store (by `kind::text`, in any status — a locally archived bullet is not resurrected) are now merged as `source: import` and persisted once; the rewritten mirror is the union. Job: check out a repo with a `.forge/MEMORY.md`, its bullets reach the prompt on this machine too.
- **`npm test` no longer runs against the real `~/.forge`**: the suite spawned agent loops with the developer's `FORGE_HOME`, so one run left 78 fixture session dirs (`cwd` under `.tmp/forge-*`), background-task and tool-output entries in `~/.forge`, re-saved the real project-memory store (sweep archived a duplicate) and rewrote the tracked `.forge/MEMORY.md` mirror from it. The `test` script now sets `FORGE_HOME="$PWD/.tmp/forge-home"` beside `TMPDIR` (gitignored). Not covered by this: `HookRunner` still reads `~/.cursor/hooks.json` via `$HOME` and Cursor's native shape (`{command, matcher?, timeout}` per entry, no `hooks[]`) makes `run()` throw `m.hooks is not iterable` — that is a product bug in the Cursor-compat loader, its own fix.
...
### Fixed
- **Capped ULW no longer spends mill units**: `max_waves` is a budget for distinct surfaces. Same-surface / chrome / pin ships hold (do not increment `w`) even when a cap is set; `/cycle 0` N+1 still finishes. Job-delta credit is language-agnostic — Python `assertIn` on stdout/stderr/help is pin-only (not a job); a production file whose diff is string-literal/TTY plus those pins is chrome. Isolates (`python -m unittest …TestCase.test_*`, `node --test tests/foo.test.ts`) are proof=ran, not proof=✓; consolidation/LAST require the project suite (timeout = ✗, do not skip). Evaluate-class garnish repeats from the dirty tree after wave 3 (`soulNudgeDone` cannot waive the cap). LAST reflect fills Must-fix from the ledger; `Must-fix: none` is illegal when those holes exist. Maze speak-once mill still holds; a real control-flow ship still increments `w`.

### Added
- **Independent subagents overlap in one round**: explore/plan and worktree `general-purpose` batch with `web_search`/`read_file` (cap 8). Explicit `isolation=none` GP stays a sequential barrier. Land is serial original-order (`OrderGate`; every spawn ticket `finish`es in `prepareToolResult` `finally`, including deny/skip/abort). PLAN/orient omitted `subagent_type` is **explore**; explicit GP still denies under `ulw_orient` with a `Next:` to retry as explore in the same round (LAST still denies all spawn). `FORGE_SUBAGENT_PARALLEL=0` off. Worktree MCP/LSP autostart opt-out: `FORGE_SUBAGENT_CHILD_MCP=0` (`mcpAutostart`/`lspAutostart`, not `mcp: undefined` alone). Job: Wave 1 PLAN emit two explores + two searches, they overlap; omitted type prints `explore:` not `general-purpose:`.
- **Incomplete children continue instead of dying at the cap**: explore/plan default **25** turns, general-purpose **80** (`FORGE_SUBAGENT_MAX_TURNS` still overrides all). ~80% wrap poke; last turn is report-only (new search/edit skipped). Handoff includes typed **Next:** — implementers `resume_session_id`, explore uses `pick:` / do not re-explore. `spawn_subagent({ resume_session_id })` continues the kept session (fresh budget). Explore ledger skip is `skipped_explore_ledger` (`ok: false`), not a successful look. Cite-delta stop requires `pick:` or an ignored poke (tools-only after poke gets one report-only turn). Doom-loop is outcome-aware: success repeats trip at 2; MCP `partial` / stubs / `get_task_output` without `wait=` get specific Next, not generic STOP. `search_mcp` waits for idle/connecting servers instead of returning a half catalog. Job: a child that hits maxTurns can be resumed; `search_mcp *` ×3 after a full catalog still stops, connecting MCP does not.
...
### Fixed
- **Credential dump leftovers closed**: `~/.grok/auth.json` is a live xAI session and was still readable. Bash now realpaths dests (symlink `cat escape`), treats `cp`/`dd if=`/`python -c`/`node -e` sources as reads, parses `cp -t DEST`, and `toolBash` re-checks reads (not only writes). Seatbelt/bwrap hide Grok auth + private-key basenames (`id_rsa$` so `.pub` stays). `git config core.hooksPath` (and `git -c core.hooksPath=`) is hard-denied; missing `.git/hooks` is created so Linux bwrap can ro-bind it. keepSecrets allowlist is `GITHUB_TOKEN` / `GH_TOKEN` / `CONTEXT7_API_KEY` — AWS/DATABASE_URL no longer inherit into Playwright. Family live-fold never drops a running child (re-fold was double-counting). Incomplete worktrees keep even under `land=discard`. Background journal stamps the launch turn. REPL `/config` `/provider` `/doctor` issue body, posture `↳ review`, and provider-error tips no longer dump `forge login` at ›. Job: `python -c`/`cp`/`cat escape` of auth.json refuse; `/undo` of the launch turn restores a bg write that finished later.
- **Credential files cannot be dumped into the model**: write protection and `keepSecrets` already stopped overwrite and MCP inherit of `XAI_API_KEY`, but `read_file` / `grep` / `cat` of `~/.forge/auth.json` (and `~/.ssh/id_*`) still sent tokens to the LLM — YOLO and `--read-outside allow` skipped only the ask. File tools and bash readers now hard-deny those paths (`isProtectedReadPath`; inspecting a hook still allowed). The permission gate uses `hardSafetyCheck` for every tool, not only bash. Seatbelt denies file-read of `auth.json`; bwrap binds `/dev/null` over it. Designed leftover: `python -c` when `sandbox=off`, and Linux hide skips a missing file. Workspace `.env` stays readable. Job: `read_file ~/.forge/auth.json`, get a refuse, not the key.
- **MCP/LSP stdio no longer inherit Forge LLM keys**: `createChildEnv({ keepSecrets: true })` kept every host `*KEY*`/`*TOKEN*` so built-in `playwright` / `context7` (and any `npx` MCP) received `XAI_API_KEY`, `OPENAI_API_KEY`, `CURSOR_ACCESS_TOKEN`, … — the same class as leaking `auth.json`. `keepSecrets` still passes `GITHUB_TOKEN` / `CONTEXT7_API_KEY`. Provider credential names (shared `PROVIDER_API_KEY_ENV`) are always stripped from inherit; `mcp.json` `env` overlay (`set`) can still pass a key on purpose. Auth resolve, `/accounts` env-active, and `/provider` summary use the same table. Job: spawn playwright MCP, it cannot see `XAI_API_KEY`.
- **Workspace sandbox no longer writes `.git/hooks` or `~/.forge/auth.json`**: default `sandbox=workspace` allowed the whole CWD + `~/.forge`, so `python` / `sed -i` / `node` could install a hook or overwrite credentials when `write_file` and `extractWritePaths` (redirect/tee/cp) could not see the dest. Write allow is now CWD + `~/.forge/{sessions,logs,tmp}` + temp. Seatbelt deny-after-allow covers `.git/hooks` / `.ssh` / `.gnupg` / shell rc / `id_rsa` (git commit/config stay allowed — `.git/config` and `HEAD` stay writable). bwrap binds forge slices only and ro-binds existing hooks/ssh. Background bash uses the same `seatbeltProfile` (the duplicate writer was stale). Designed leftover: Linux cannot ro-bind a missing `.git/hooks` (creating the dir still works until it exists; Seatbelt on macOS covers create). Job: default sandbox, `python` cannot write `pre-commit`.
- **`isolation=none` child writes join the parent undo journal**: same-workspace `spawn_subagent` journaled only on the child session; a completed child `deleteSessionDetailed`s that journal, so `/undo` and `/files` stayed blind while the parent tree stayed mutated (worktree land already journals parent pre-images). Child `mutations.jsonl` now folds onto the parent at the spawn turn before cleanup. Designed empty: no child journal / `isolation=worktree` (land owns that) / same session id. Job: type `/undo` after an in-place spawn, get the tree back.
- **Forge git helpers no longer inherit host `GIT_DIR`**: bash already used `createShellEnv`; checkpoint, journal, worktree land, `/commit`, `/diff`, auto-commit, format-on-write, `lsp ensure`, hooks, grep, and MCP/LSP stdio inherited raw `process.env`. A husky hook or `GIT_DIR` in the parent shell redirected those git calls (or injected `NODE_OPTIONS` into prettier / `npm install -g`). New `createChildEnv` scrubs injection + secrets; checkpoint temp `GIT_INDEX_FILE` is policy `set` after the scrub (not `{...process.env, ...extra}`). MCP/LSP stdio keep host tokens (`GITHUB_TOKEN`) so configured servers still auth; injection is still stripped. Job: launch forge from a git hook, `/checkpoint` and `/undo` still talk to the workspace repo.
- **`lsp({ action: "ensure" })` is a mutation, not a free read**: the tool runs `npm install -g` / rustup / go install, but the gate treated all `lsp` as read-only — so `/plan`, ULW Wave-1 PLAN (even under yolo), dontAsk, default/headless, and session-tool on `lsp status` could install globally without asking (same class as `web_fetch allow_local`). Plan / orient deny ensure; headless/dontAsk need an allow rule, pattern-always, or YOLO; session-tool on status does not cover ensure. diagnostics / status / install-guide / `dry_run` stay read-only. CLI `forge lsp ensure` is unchanged (user-initiated). Job: type `/plan`, `lsp ensure` cannot install.
- **`apply_patch` rollback no longer blocks the retry as "changed on disk"**: restore rewrites mtime; the file-read guard then refused the same pre-image. Rollback now restamps noted paths (`refreshNotedFromDisk` — does not invent a read). **Sibling:** `spawn_subagent` copies the pre-worktree family pin onto the child — a second `pinChildCostCap` after a sibling live-fold could refuse and leave `child.meta` uncapped (fresh `config.maxCostUsd`).
- **`apply_patch` is a transaction**: a later op that threw left earlier files written (`partially applied` / `NOT rolled back`). Validate-then-write is not atomic. Mid-apply failure now restores pre-images (add→unlink, update→before, delete→rewrite, move→restore src + drop dest). Journal / `onEdit` only after the batch commits. Designed leftover: empty parent dirs from a rolled-back add. Job: one patch, all files or none.
- **`/budget` is a family spend cap**: `spawn_subagent` inherited `config.maxCostUsd` on a fresh child session, so each explore got a new $5 while the parent was already at $4.90 — three parallel children blew the sit-down valve, then `foldChildUsage` made the parent HIT after the money was gone. Children now pin `meta.maxCostUsd` to **remaining**; parent HIT refuses spawn (before a worktree). Live-fold each child turn so siblings share the remaining. Cost-cap release is `incomplete_cost_cap` (does not land). Designed empty: no cap / remaining covers the child. Job: type `/budget 5`, spawn cannot outspend it.
- **Incomplete worktree children no longer land into the parent**: `isolation=worktree` used to apply whenever `editCount > 0`, so `incomplete_max_turns` (and a blocked SubagentStop) dumped a half-finished tree into the parent and **deleted the worktree** — the only copy. Land now requires handoff `status=completed`. Max-turns / abort / error / stop-hook / cost-cap skip apply and keep the worktree for recovery. Completed children still auto-land (`FORGE_SUBAGENT_LAND`; `/undo` reverts). Job: a child that hits its turn cap must not mutate the parent.
- **`/files` and `/last` show bash / background / land writes**: `listSessionTouchedFiles` only parsed tool-call `path` args, so a bash-only session opened `No file mutations recorded` while `/undo` and `/diff` told the truth (bg `python -c` / worktree-land have no `path` field). The card now merges `mutations.jsonl`; journal abs paths collapse onto the workspace-relative tool-call key. Designed empty: no journal / no path args / `FORGE_BASH_MUTATION_JOURNAL=0` / still-running bg (until exit). Job: type `/files` or `/last`, see what landed.
- **Background bash writes join the undo journal**: `bash { background:true }` used to skip `mutations.jsonl` (designed empty: background), so `/undo` restored `write_file` and foreground/`!cmd` and left `python -c` / codegen on disk. Snapshot at start; porcelain delta applies on exit (failed writes still journal). Paths already journaled after the snapshot (a concurrent `write_file`) stay that tool's entries. `/undo` of the launch turn settles the in-flight journal and SIGKILLs those writers so they cannot recreate undone files. Designed empty: not a repo / clean tree / no `recordMutation` / still running (until exit or settle) / `FORGE_BASH_MUTATION_JOURNAL=0`. Job: type `/undo`, get the tree back — including the bg codegen.
...
- **Bash writes join the undo journal**: `echo >` / `python -c` / codegen via foreground `bash` (and idle `!cmd`) used to skip `mutations.jsonl`, so `/undo` restored `write_file` but left shell writes on disk. Git porcelain delta around the command journals create/update/delete (pre-bash body when already dirty, HEAD when clean). Designed empty: not a repo / clean tree / no `recordMutation` / still running (until exit or settle) / `FORGE_BASH_MUTATION_JOURNAL=0`. Background bash now journals on exit (same porcelain delta). Failed commands that still wrote are journaled. Idle bang is its own turn so `/undo` 1 undoes only that write. `/verify` sets `journal: false` so a check does not become an undo turn or journal test fixtures. Job: type `/undo`, get the tree back.
