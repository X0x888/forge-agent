...
- **ULW `max_waves` vs mid-loop stamps**: unattended edit bursts no longer increment the user-facing wave counter (so `w=7/2` cannot happen). Idle 20-step epochs still stamp a wave and **honor the cap** (auto LAST + admit). HUD tags show `w=N/M` (current/cap) instead of `mw=M` only. Live hint is `last=/cycle 0` so it does not look like the current cycle. Resume words (`continue`, `keep going`) no longer re-arm ULW or replace the mandate with a 5k god-mode dump. First wave after edits is `net=new` when the first fingerprint is the baseline.
