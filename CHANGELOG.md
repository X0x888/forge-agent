...
### Added
- **Native forge-* skills (ship-with-install)** — package `skills/forge-*/SKILL.md` playbooks load as `source: builtin` (project > user > builtin). Progressive prompt injection: full catalog + paths; bodies for project/user and `forge-method` (`inject: always`); other builtins catalog-only (`read_file` when matching). `/skills` groups by source. `FORGE_BUILTIN_SKILLS=0` disables. Pack: `forge-method`, `forge-shape`, `forge-blueprint`, `forge-march`, `forge-redgreen`, `forge-rootcause`, `forge-prove`, `forge-inspect`, `forge-assay`, `forge-swarm`, `forge-anvil`, `forge-ship`, `forge-armor`, `forge-absorb`, `forge-craft`.
