...
- **`releasedOnContinueCap`**: headless `forge run --json` + metrics flag when the stop-continue safety valve releases (length / content_filter / empty / Stop-block) — CI can alert without hard-failing
