...
- **Cursor native-tool reject names Forge editors**: rejected AgentService Write/StrReplace told the model to “use the MCP tools”, so Cursor Grok `search_mcp`’d for `write`/`edit` (30s timeouts) instead of `write_file` / `search_replace` / `apply_patch`.
