...
- **Cursor AgentService model knobs**: Forge `/effort` and `/context-window` now map onto Cursor `RequestedModel` parameters + `ModelDetails` (`thinking_details`, `max_mode`). Thinking stays **off** unless the id has `-thinking`; context ≥1M sets Max Mode; effort defaults to each model’s max (`claude-fable-5` → 1M + max). Aliases: `fable` / `fabel`. Variant suffixes (`-fast`, `-high`, `-xhigh`) are not treated as catalog typos. Cursor’s default model is **`grok-4.6-high-fast`** (High + Fast); an effort suffix on the id is the default, not Grok’s xhigh cap.
