...
- **`max_waves` is a ceiling**: evidenced **Cycle complete.** under `cycle=1` (reading + proven work) flips LAST and releases. A red check does not count as that evidence. The closing wave is stamped (`w=1/N`, not `w=0/N` on the auto-commit). Yield (“shall I continue?”) is still handoff-blocked. Kickoff/admit say unused slots are not work to invent.
