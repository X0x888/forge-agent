...
### Fixed
- **Unattended cost: Forge talking to Forge**: mid-conversation admits no longer fire when only ship-logs/readings change (`decisionsFp` is durable memory only). Stop re-anchors mark the snapshot admitted so the next boundary does not emit a second “Obey this state.” ULW kickoff skips the prompt-start admit. Request prune stubs older harness user pokes (keep the latest admit + Stop). Stop re-anchors dropped the 3.5k decisions dump.
...
- **`max_waves` is a ceiling**: evidenced **Cycle complete.** under `cycle=1` (reading + proven work) flips LAST and releases. Yield (“shall I continue?”) is still handoff-blocked. Kickoff/admit say unused slots are not work to invent.
