...
- **`github` tool** (`src/agent/tools/github.ts`): search repositories/code, repo metadata, file contents, README, tree. Public repos work without a token; `GITHUB_TOKEN` / `gh auth login` raises limits and unlocks code search. Tokens never enter the output. `web_fetch` rewrites `github.com/.../blob/...` to `raw.githubusercontent.com`. `FORGE_GITHUB=0` off. Job: read a library's source instead of googling its marketing page.
