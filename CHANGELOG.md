...
### Changed
- **ULW: kickoff, admit, and README match the driver lifecycle** (`src/harness/cycle/index.ts`, `src/harness/context-admit.ts`, `src/harness/cycle/briefs.ts`, `README.md`, `docs/ULW.md`, `skills/forge-reviewer`). Unlimited copy no longer says a no-mandate run ends when the product looks fine — release is a mandate `fulfilled`, Planner `blocked`, `/cycle 0`, `max_cycles`, or the no-progress wall. Reviewer turn 2 is the document; Must-fix is how the tree changes. Job: kickoff/admit/README no longer tell a no-mandate run it can stop because the product looks fine; Reviewer turn 2 is the document.
