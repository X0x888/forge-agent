...
- **Click-to-caret and dock chips** (`src/tui/mouse.ts`, `src/tui/prompt-editor.ts`, `src/tui/bottom-status.ts`): SGR mouse (press only; `FORGE_MOUSE=0` off; Shift-click ignored). Click in the prompt places the caret on a grapheme, including the right half of 你. The dock `stop` chip aborts the turn (same as Ctrl+C, not `/cycle 0`); after abort, `resume` is `/retry`; model / ULW / auth chips open `/model` / `/cycle` / `/accounts`. Job: click `stop` while it thinks; click in `hi你好` to sit on 好.
