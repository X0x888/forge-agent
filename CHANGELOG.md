...
- **ULW verify is a process that exits** (`src/harness/cycle/verify.ts`, `declared-checks.ts`, `orchestrator.ts`): `Verify: … && npm run preview` never returned (Vite held pipes; `'close'` never fired; process-group kill was bash-only). Verify now spawns its own group, settles on `'exit'`, process-group-kills on timeout, refuses preview/dev/watch/serve, keeps the finite prefix of a compound (`build:ea && preview` → `build:ea`), harvests `mkdir && swiftc HostCareCheck`, and an `.xcodeproj` tree lists `xcodebuild` so a software workspace is not eight cycles with `verifyCommand: null`. Job: word-games' preview is not the gate; pixel-pets' HostCareCheck is.
...
- **Verify timeout reaps session GUI** (`src/agent/loop.ts`, `src/agent/browser-lease.ts`): reap never ran while verify hung, leaving Vite `:4173`, `simctl` glance, native `.build/debug` bins, and a 199 MB look profile. `simctl` / `vite preview` / workspace native bins register as GUI leases; verify timeout reaps the session. Job: a hung preview does not leave `:4173` after the gate times out.
