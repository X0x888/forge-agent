...
describe("cycle orchestrator", () => {
  ...
  it("turn start: the Planner writes cycle 1, items seed the board, the plan is admitted", async () => {
    ...
    assert.deepEqual(calls, ["role:planner", "todos:3"]);
  ...
  it("EXECUTE Stop with open items re-anchors; 'Plan complete.' runs review → verify → commit → next plan", async () => {
    const sid = "orch-full";
    ...
    assert.ok(fs.existsSync(path.join(cycleArtifactsDir(sid, 1), "verify.log")));
  });

  it("/cycle 0 finishes the open cycle (review, verify, commit) and then releases", async () => {
  ...
  it("red verify after review → FIX re-anchor with the tail; green on the next Stop commits", async () => {
    const sid = "orch-red";
    armWithPlan({ sessionId: sid, cwd, verifyCommand: "npm test" });
    const { rt, calls } = fakeRuntime(cwd, {
      reviewer: [REVIEW_MUSTFIX],
      checkPasses: [false, true],
    ...
    assert.match(r1.reanchor ?? "", /RED \(fix round 1\/3\)/);
    assert.match(r1.reanchor ?? "", /fail 2/);
    assert.match(r1.reanchor ?? "", /the flag prints nothing/);
    ...
    assert.equal(calls.filter((c) => c.startsWith("check:")).length, 2);
  });
  ...
  it("/replan closes the cycle at the next Stop and re-plans", async () => {
    ...
    assert.deepEqual(calls.slice(0, 2), ["role:reviewer", "check:npm test"]);
  });
