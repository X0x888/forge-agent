...
| `spawn_subagent` | Nested agent (`Task`). Types: general-purpose / explore / plan. Returns `status` (`completed` / `incomplete_max_turns` / …) plus `artifact_path` under `~/.forge/tool-output/subagent_<id>.md` — read that to restore; do not re-spawn. Hit max turns is incomplete, not success. Last turn injects a “write the report now” reminder. **`isolation=worktree`**: detached git worktree under `~/.forge/worktrees/` (requires git). General-purpose **defaults to worktree** when the workspace is a git repo (explore/plan stay in-place; `FORGE_SUBAGENT_ISOLATION=none` or `isolation=none` writes the parent). On a **completed** handoff Forge **auto-lands** the diff into the parent (`FORGE_SUBAGENT_LAND=auto|keep|discard`; kept on conflict) and journals parent pre-images so `/undo` can revert a bad land. `incomplete_max_turns` / abort / error / stop-hook **skip land and keep the worktree**. Force keep with `FORGE_SUBAGENT_KEEP_WORKTREE=1`. Depth `FORGE_SUBAGENT_MAX_DEPTH`. |
