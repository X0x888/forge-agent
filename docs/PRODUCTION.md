...
forge login -p cursor        # local import, then Cursor browser login
...
| `forge login --from-cursor` / `-p cursor` | Import local Cursor CLI auth.json / keychain (then browser login) |
...
```json
{
  ...
  "admitCount": 1,
  "proofPokes": 1,
  "guardBlocks": { "proofClaim": 1, "ulw": 1 },
  "providerRounds": 12,
  "editCount": 4,
  "openTodos": 0,
  "messageCount": 24,
...
`releasedOnContinueCap: true` means the stop-continue safety valve fired (length / content_filter / empty, or a Stop-block cap on capped/LAST/non-ULW runs). Unlimited CONTINUE Stop-blocks do not trip it. `hitMaxTurns: true` means the loop exited on `max_turns` (not a clean Stop). `hitCostCap: true` means the session spend estimate hit `max_cost_usd` / `FORGE_MAX_COST_USD` / `--max-cost` / `/budget` (estimateCostUsd only — not a bill). All three stay `ok: true` unless aborted/timed out/empty-run, so CI can alert on caps without hard-failing. `harnessUserPokes` / `admitCount` / `proofPokes` / `providerRounds` count this run's Forge-injected user-channel messages and model rounds (unattended cost meters; also on `metrics.jsonl` and last-run session meta). `guardBlocks` breaks the Stop-guard bounces down by guard — `handoff`, `proofClaim`, `report`, `todoGate`, `goal`, `ulw`, `hook` (a user Stop hook), `verify`, `fix`, `doomLoop`, `errorStreak` — non-zero keys only, so an over-eager guard is a number you can alert on, and `forge stats` totals them across runs. The same three also appear as strings inside `productionWarnings[]` post-run (grep-friendly). `lastError` (when present) is `{ at, code, message, tips? }` stamped for recovery — codes include `max_cost`, `max_turns`, `continue_cap_*`, `handoff_released`, `proof_claim_released`, auth/rate-limit failures. `finishReason` is the last provider `finish_reason` (e.g. `stop`, `length`, `content_filter`, `tool_calls`) or `null` if no model turn completed. Mid-run catch failures include `reason=error|timeout|aborted`.

Each headless/REPL turn also appends a counter-only `run_end` line to `~/.forge/metrics.jsonl` (no prompts or secrets). Per-provider-round `provider_round` lines (prompt/cache/prune per call) go to `~/.forge/rounds.jsonl` — a separate file, because one ULW run writes hundreds of rounds and while both shared one 2,000-line file the auto-prune evicted every `run_end` within a day. Both kinds are also appended to `~/.forge/sessions/<id>/rounds.jsonl` so a long run's full trail survives the global prune. `forge prune-metrics --keep N` cuts both global files.
...
- **Provider failures** — REPL prints recovery tips; `forge run --json` fail payloads include `recovery: { code, tips }` (auth/rate-limit/quota/overflow/network/DNS/overloaded/content_filter/empty_response/unsupported_feature/org_verification/model_deprecated). Session `meta.lastError` surfaces on `/status`, resume, `/share`, and `forge status --json`. Recovery backlog: `/sessions errors` · `forge sessions list --errors` · `doctor --json` `sessionsWithLastError` / `sessionsLastErrorByCode`. Doctor and the errors list glance by **code** (`45 max_turns · 8 bad_request`) so a budget-wrap backlog cannot hide 429s. `listSessions({ errors: true })` filters **before** the limit. **Prune keeps lastError sessions** by default (`skippedLastError`); only delete them with `forge sessions prune --force-last-error` after review.
