...
- **Provider failures** — REPL prints recovery tips; `forge run --json` fail payloads include `recovery: { code, tips }` (auth/rate-limit/overflow/network). Session `meta.lastError` surfaces on `/status`, resume, `/share`, and `forge status --json`. Recovery backlog: `/sessions errors` · `forge sessions list --errors` · `doctor --json` `sessionsWithLastError`.
