...
- **Picks are the named-ship list**: unlimited ULW seeds `namedShips` from explore-map picks when the model never writes a list. A pick completes only on its job (distinctive terms / claims — `takeJoinerOverHome`, climactic 5/10/13), not topic words (`online`, `joiner`, `toast`) and never FIFO. When every seeded pick is done, exhaust holds for a different-class `Reading:` or `/cycle 0`. Caps still spend. Compact reprints open seeded picks.
