...
PLAN ──► EXECUTE ──► VERIFY ──► REVIEW ──► VERIFY ──► COMMIT ──► RELEASED
 ...
 └──────────────── re-plan ◄───────────┴─────────────────────┴──────┘  (/cycle 0 · max_cycles · mandate fulfilled · blocked · no-progress)
```

- **PLAN** — a fresh-context **Planner** runs two turns on one kept session. Its scout exercises the product through a representative public interface, inspects relevant promises as `kept | broken | absent | unknown | limited`, and compares credible improvements or consequential investigations with `leave it`. Discovery considers the whole product without requiring a candidate from every category. The plan turn then receives the record, user interjections, and spend, strikes completed work, and writes one coherent cycle. A labelled plan buried after a budget prefix is salvaged; architecture hold is satisfied by `leave it` that names the class, or `Out of scope:` that names it.
...
- **VERIFY** — the **harness itself** runs a whole project check. Preview/dev/watch/serve is not a gate (`finiteCheckCommand`); a compound keeps the finite prefix (`build:ea && preview` → `build:ea`). The check settles on `'close'` so the suite summary drains; on timeout it process-group-kills and settles on `'exit'` (orphaned pipes never `'close'`). A Planner's isolate is recorded as proof=ran and the stack's suite gates instead. New named failures are red relative to the command's eligible baseline; pre-existing failures are reported. Named failures that cite `.forge/MEMORY.md` or `AGENTS.md` are operator-file greps, not product-gate red. Cycle 1 measures the user's tree. Later baseline refresh requires a committed prior cycle and a confirmed clean repository root. A red run without named failures remains red. Verification and review repairs share `fix_rounds` (default 3).
- **REVIEW** — on a tree that passes the gate, a fresh-context **Reviewer** first exercises the intended workflow or condition without the diff and writes `look.md`. A docs-only cycle (MEMORY.md / README / look junk, no product path) skips that look turn and writes a stub `Looked: skipped — docs-only`. It then reads the plan, cumulative cycle diff and prior record and writes `review.md` (`documentOnly`; Must-fix is the revision channel). It judges fulfillment, acceptance defects, architecture and evidenced benefit. An explicit `blocked` verdict or an unsuccessful, incomplete or unparseable review closes without a commit; the next Planner receives its findings. A labelled `Verdict:` buried after a budget prefix is salvaged; `Goal achieved.` is not a review. A labelled look/fulfillment without `Worth:` salvages as `blocked` (`salvageIncompleteReview`) — the look is kept, the stub is not. A surface-claim cycle (popup / first-hour / browser / watch / glance / HUD sit) whose look never opened the product (`could not run`, Playwright never initialized, dropped image attachments, Vision dropped) cannot commit on a `ship` (`FORGE_ULW_LOOK_GATE=0` off); CLI/JSON-RPC proofs and a kernel/API look (`walk()`, `--self-test`, `SMOKE_*`) still ship. `simctl` without seen pixels is not kernel evidence. `/cycle 0` with items done skips a dead look when the review itself labelled `Verdict:`. Repairable defects use `ship-with-revisions` with `Must-fix` and require same-cycle repair, as described below.
- **VERIFY again** — the check runs once more after the review; red invalidates review approval and goes back to the executor (same `fix_rounds` budget). After repair, the harness verifies, runs a fresh Reviewer, verifies again, and only then commits. A nominal shipping review with unresolved `Must-fix` or partial/missing items also enters this bounded repair sequence. Failed or incomplete role results cannot approve work, even if their text contains a shipping verdict. Prior reviews are retained as `review.before-fix.<round>.md`.
- **COMMIT** — one local commit per reviewed, green cycle (`ulw cycle N: <title>`; never pushed; `FORGE_ULW_AUTO_COMMIT=0` off). While ULW is armed the executor cannot `git commit` (`FORGE_ULW_DRIVER_COMMIT=0` off). If the workspace is not a git repository and best practice would start one (a software tree, or any ULW commit), the harness runs `git init -b main` and writes a starter `.gitignore` (`FORGE_AUTO_GIT=0` off). A later gate change may capture a fresh baseline only after a committed cycle with an independently confirmed clean repository root, including files outside a nested workspace. Otherwise the previous baseline remains and the changed command must pass; rejected or dirty work cannot become a new baseline. Cycle 1 still measures the user's initial tree, including existing edits. Docs-only commits do not reset the no-progress wall. A mandate `fulfilled` without sitting the product (`Looked:` that only read the README, or no Looked: and no substance commit) is treated as go-deeper, not a release.
- **Re-plan** — a fresh Planner reads the previous plans, reviews, the Reviewer's `Must-fix`, unfinished items and anything the user typed since the last plan. A **mandate** `Verdict: fulfilled` ends the run; `/cycle 0` or `max_cycles` ends it after the commit. `/cycle 0` typed while that next Planner is running (the previous cycle already closed) stops without admitting another plan — the sidecar write is overlaid onto the orchestrator's in-memory copy so a later persist cannot clobber it. On a **no-mandate** run the model does not get to stop itself: a `fulfilled` becomes deeper work (below), and a Planner that cannot produce a plan becomes work rather than a release (a parseable scout is admitted only when `cycles.length === 0`; after cycle 1 the plan turn always runs, and a starved Planner synthesizes — keep-promise / go-deeper / direct-execute).
...
- **The Planner could not produce a plan.** A scout that already parses as a `continue` plan is admitted only when `cycles.length === 0` (no record to miss). After cycle 1 the plan turn always runs; if it fails after one retry with the plan turn still `documentOnly`, unkept promises become a **keep-promise** cycle, all-kept promises a **go-deeper** cycle, and an empty inventory a **direct-execute** cycle. The executor selects an evidenced improvement or investigates a consequential uncertainty, instead of ending the run on a missing document.
- **The Planner declared a no-mandate product "done."** A no-mandate `fulfilled` continues: broken or absent promises prompt repair, and `unknown` promises prompt investigation before deciding whether repair is needed. Otherwise a **go-deeper** cycle investigates an untested core workflow or consequential risk through the appropriate app, CLI, library, or service interface. Finding no justified edit is a valid investigation result. An explicit mandate is fulfilled only when broken and unknown promises are kept or named on `Operator:`; a class two shipped reviews named must be an item or `leave it` on the next continue plan; `/ulw` after an ended run starts a new report epoch (`report-2.md`).

The only self-stops the harness honours are a mandate fulfilled after sitting the product, the user's controls, and — with **no mandate** — the **no-progress wall** on repeating empty work: when `directExecuteStreak` consecutive synthesized cycles land no commit, or `noCommitStreak` consecutive cycle closes land no substance commit (not a dead look lease, not `FORGE_ULW_AUTO_COMMIT=0`) (`FORGE_ULW_NO_PROGRESS_CAP`, default 3). An open mandate is the job: empty cycles keep planning. A Planner `blocked` or a look that could not run synthesizes other work — it does not end the mill, and look-lease synths do not burn `synthStreak`. An unpaid skip restores the tree to `cycleStartHead` (`FORGE_ULW_RESTORE_UNPAID=0` off) so the next cycle is not grinding leftover files. A dead Playwright child is not `ready`: its exit drops the tool registry (the scout brief says down, not connecting); reconnect rotates `browsers/look-N`; `look_native` screenshots harness Chromium of the look server on demand (never the TUI; plan admission does not spawn a second Chrome). A substance commit resets both streaks. This is non-negotiable 2 ("never an infinite trap *without progress*") read as: do not loop the same empty class; do not abandon a mandate because a lease died or a cycle shipped nothing.

The other reading is the mill itself: keeping it up is the design, not a bug later work should "fix." A dogfood that ran all night, spent many tokens, and shipped little or poor work is a **work** defect — the Planner chose the wrong item, the look was not a sit, `Worth:` did not hold, verify was an isolate, a brief wasted a role's turns. Improve those so the hours pay. Do not add a default `max_cycles`, lower `FORGE_ULW_NO_PROGRESS_CAP`, fuse on spend, or sit the mill down early. `/budget` and `/max-cycles` are operator valves, not quality substitutes. The no-progress wall is "do not loop the same empty class," not "the mill ran long."

**The waste that made the escape look reasonable is gone too.** A role that cannot edit (the Planner: `denyEdits`) no longer hears the "fix until green" or "run the check" nudges — the HashPet Planner ran the suite, went red, was told to "fix the root cause … until green," and burned ~45 of 60 turns chasing a failure it could not touch. The signal is the role's own tool set (`toolSetCanEdit`): no edit tool, no fix nudge. The Planner's **plan turn is document-only** (`documentOnly`): the scouting is done on turn 1, so turn 2 emits the plan and cannot re-enter reading. A scout that already is the plan skips that turn only when `cycles.length === 0`; cycle 2+ always runs the plan turn. While ULW is armed the driver owns `git commit` (`FORGE_ULW_DRIVER_COMMIT=0` off). An unparseable review is retried once. `Verify:` compounds that include a product smoke (`godot --headless --script`) stay the gate; `cargo test -p` alone is an isolate. Architecture-class hold on a synthesized cycle is a `leave it` in Considered:, not a fake item 0 (three shipped reviews still owe a collapse item).
...
| `forge-peer-scout` the category bar from peers | A background explorer sits 1–3 maintained GitHub peers of this Identity's job while EXECUTE runs; staged `peers.md` reaches the **next** Planner after `Looked:` as evidence, never as items. The executor and the Reviewer never see it. Fail-open; `FORGE_ULW_PEERS=0` off. | `peer-scout.ts`; scout brief; `/cycle peers` |

Presence and shape are parsed; content is judged by the next role and by whoever opens `cycles/<n>/`. A `continue` plan missing nonempty `Looked:` / `Direction:` / `Worth the cycle:` (a failed sit is a look; empty Looked is not), `Considered:` (with `leave it`), an item's `serves:` / `red now:`, more than twelve items, exactly one item without `One item:`, two or more `Out of scope:` entries on a one-item plan, or numbered item lines under `One item:` comes back to the Planner once with what was missing, on the kept session without re-scouting. Fulfilled/blocked stay exempt. A review without `Worth: yes|no` does not approve (a labelled look without Worth salvages as blocked, not a blank stub). A scout without nonempty `Looked:` is incomplete. A second failure becomes a synthesized work cycle, subject to the no-progress wall. `FORGE_ULW_TWO_TURN=0` runs each role on one brief; a runtime that cannot keep a session, or a resume that fails, falls back to the same single brief with the scout inlined.
...
| **b — direction** | `/ulw polish the first-run experience` | The direction frames every plan; the run cycles until mandate `fulfilled`, `/cycle 0`, `max_cycles`, or the no-progress wall on repeating empty work. A Planner `blocked` keeps going on other work. |
...
Invention and repair are both legitimate in every case; the tree decides which the cycle needs. There is no Bet contract and no mandate classifier. The user's words are passed through verbatim — they are **attention**, not a spec and not a quality ceiling. A laundry-list `/ulw make it more interesting, attractive, addictive, ship-ready` does not license a laundry-list cycle, and `/ulw add --version with a test` does not become a product rewrite. The Planner translates after using the product and knowing the category's bar (a shipped `forge-*` skill, a peer scout note if one exists, or `web_search` if it does not already know). `Direction:` is that sentence. The Reviewer judges a demanding user of this product, not whether the diff matches the mandate's adjectives.

## Peer scout

A read-only harness sibling (`peer-scout.ts`) researches the category off the scout's clock. It arms from a README/kind hypothesis, locks when the first scout writes `Identity:`, and spends the EXECUTE / VERIFY / REVIEW window on 1–3 maintained GitHub peers of **this job** (`github` `sort=stars` as a junk filter, then `readme` / the matching surface). Staged notes land in `~/.forge/sessions/<id>/peers.md` (and a copy under `cycles/<n>/`). The **next** Planner reads them after `Looked:` — the same mailbox as `Serendipity:`. The current plan stays the contract: the executor transcript is not a place for "helix does X". The Reviewer does not grade the diff against a famous repo. Stage 3 + stable Identity → sleep. Identity change → re-aim. Plan admission never waits. `/cycle status` shows the stage; `/cycle peers` shows the note; the dock paints `peers 2` while it runs. `FORGE_ULW_PEERS=0` off.

## Controls

```text
/ulw [mandate]         arm (bare /ulw = case c)
/cycle 0               finish this cycle (execute → verify → review → verify → commit), then stop. After a commit, while the next Planner is running, the run stops without starting that plan
...
Worth the cycle: <whether this session beats leave it — the scout already spent; pack the class>
...
2. <same theme — pack the class so the executor finishes it in one session; more than 12 is a backlog>
One item: <required when Items has one entry, after every Items: row: why the other Considered candidates are a different job or leave-it, not sequels>
Out of scope:
- <a different job or a lease limit — not the rest of this Looked: class parked for cycle N+1; a one-item plan may keep at most one entry>
...
`Verify:` goes through the strict check-command harvest (`looksLikeCheckCommand`, then `finiteCheckCommand`): prose is refused, preview/dev/watch/serve segments are dropped so the gate is a process that exits, `none — why` falls back to the stack table so a wrong "none" cannot switch the gate off. `Guidelines: fix: …` becomes the plan's first item. A `continue` plan with no items, with more than twelve items, with exactly one item and no `One item:`, with two or more `Out of scope:` entries on a one-item plan, with numbered item lines under `One item:`, without nonempty `Looked:`, `Direction:`, and `Worth the cycle:` (a failed sit is a look; empty Looked is not), without `Considered:` carrying a `leave it` entry, or with an item lacking `serves:` or `red now:` does not parse; the Planner is retried once with what was missing named (`explainPlanParseFailure`), then a synthesized work cycle continues from the available evidence, subject to the no-progress wall. The cycle is one executor session of a theme, not one tiny task wrapped in a fresh Planner and Reviewer. More than twelve items is a backlog. `Out of scope:` is a different job or a lease limit, not a sequel list. After a one-slice cycle whose `Considered:` already held the class, the Reviewer writes `Worth: no` and the harness packs those siblings onto this cycle's board (`FORGE_ULW_SLICE_PACK=0` restores skip-and-replan). A packed cycle that was not worth it still does not commit. After a skipped `Worth: no`, the same `Direction:` (whitespace-collapsed, case-insensitive) does not admit — one retry, then go-deeper unless promises are unkept (`FORGE_ULW_WORTH_HOLD=0` off). Fulfilled/blocked stay exempt. A review without `Worth: yes|no` does not approve (a labelled look without Worth salvages as blocked, not a blank stub). A scout without nonempty `Looked:` is incomplete. `Looked:` is the Planner's own record of having used the product before judging it — read the cycles' `Looked:` lines in a row to see whether a run planned from the product or from `grep`; read `Considered:` in a row to see whether `leave it` was ever weighed. In single-brief mode the plan carries a `Promises:` section too, since there is no scout.

**What the Planner is handed** (`briefs.ts`). Turn 1: workspace, mandate, identity, the promises as last recorded (to re-inspect, never copy), the tree, the stack table, steps 1–6 and the scout contract — **none of the record**. Turn 2: its scout, then the record — a one-line-per-cycle ledger of what this run has shipped (title · direction · review verdict · worth claimed · worth found · commit · item count), the last review's `Must-fix` and `Architecture` notes, its `Worth: no` if it gave one, unfinished items, the executor's `Serendipity:` lines (what it noticed inside the work and left alone, newest cycle first, harvested from its closers at every Stop — evidence to weigh, not orders) and `Dispute:` lines (a Reviewer revision it can show was wrong, with the evidence), what the user typed since the last plan, the run's commits, the spend so far, the guideline survey, steps 7–11 and the plan contract. **Not** the previous plan's body or its `Out of scope` list — a 30-cycle HashPet run continued the last plan's theme and its out-of-scope backlog for nine cycles of one-string renames. The record tells the Planner what is done; every cycle starts from the product, and now it has to: the scout is written before the record exists in its transcript.
...
|------|-------|-----------|----------------|-------|--------|
| Planner | runs, never edits: full `bash` + `web_search` / `web_fetch` / MCP (browser via playwright) + `spawn_subagent` explore; the file-editing tools are removed (`denyEdits`) | none | `forge-planner`, `forge-veteran`, `forge-rootcause`, plus one tree-detected category skill (`src/util/product-kind.ts`) | scout (`FORGE_ULW_PLANNER_MAX_TURNS`, 60) then plan (`FORGE_ULW_PLANNER_PLAN_TURNS`, 12, document-only) on the same kept session; scout-as-plan skips turn 2 only when `cycles.length === 0` | `[ulw] planner_model`, `planner_effort` |
| Reviewer | look may drive the product; review turn is `documentOnly`, no spawn | none (Must-fix revises via the executor) | `forge-reviewer`, `forge-veteran`, `forge-rootcause`, plus one tree-detected category skill (`src/util/product-kind.ts`) | look (`FORGE_ULW_REVIEWER_LOOK_TURNS`, 25) then review (`FORGE_ULW_REVIEWER_MAX_TURNS`, 80) on the same kept session; one parse retry | `[ulw] reviewer_model`, `reviewer_effort` |
...
Env: `FORGE_ULW=0` (driver off), `FORGE_ULW_AUTO_COMMIT=0`, `FORGE_ULW_VERIFY_TIMEOUT_MS` (20 min), `FORGE_ULW_FIX_ROUNDS`, `FORGE_ULW_STUCK_THRESHOLD`, `FORGE_ULW_NO_PROGRESS_CAP` (3 — synthesized-cycle and no-commit walls), `FORGE_ULW_SYNTH_CAP` (2 — empty synthesized cycles cap; substance synths do not burn a slot), `FORGE_ULW_PLANNER_MAX_TURNS`, `FORGE_ULW_PLANNER_PLAN_TURNS` (12), `FORGE_ULW_REVIEWER_LOOK_TURNS` (25), `FORGE_ULW_REVIEWER_DOCUMENT_TURNS`, `FORGE_ULW_REVIEWER_MAX_TURNS`, `FORGE_ULW_TWO_TURN=0` (one brief per role), `FORGE_ULW_PROMISE_FULFILL=0` (mandate `fulfilled` ignores unnamed broken/unknown promises), `FORGE_ULW_CLASS_HOLD=0` (architecture recurrence off), `FORGE_ULW_WORTH_HOLD=0` (Worth: no Direction hold off), `FORGE_ULW_SLICE_PACK=0` (Worth: no class-slice packs Considered siblings into this cycle; off = skip-and-replan), `FORGE_ULW_MAX_CONTINUES`.
