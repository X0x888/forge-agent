...
| Reviewer | look may drive the product; review turn is `documentOnly`, no spawn | none (Must-fix revises via the executor) | `forge-reviewer`, `forge-veteran`, `forge-rootcause` | look (`FORGE_ULW_REVIEWER_LOOK_TURNS`, 25) then review (`FORGE_ULW_REVIEWER_MAX_TURNS`, 80) on the same kept session; one parse retry | `[ulw] reviewer_model`, `reviewer_effort` |
