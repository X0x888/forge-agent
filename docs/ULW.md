...
| **Same-surface hold** | 3 consecutive declared ships on the same theme (summary overlap, leftover-sibling language, **or** adjacent-share / factory schema — rotating nouns on “beside you / far stays”) → unlimited ULW holds until a different-**class** `Reading:` or `/cycle 0`. Stuck-wall does not increment. Capped and `/cycle 0` N+1 budgets are not held. A new noun is not a new surface. |
...
- **Same-surface hold**: 3 declared ships on the same theme (token overlap after ritual-strip, leftover/fix-that-only language, or adjacent-share / factory schema) block unlimited ULW until a different-class reading or `/cycle 0`. Same-surface leftovers do not increment `w`. Consolidation closers do not increment or reset the streak. Capped runs and a scheduled `/cycle 0` N+1 budget are not held. `maybeAdoptNamedShips` refuses a one-ship mill reading after exhaust. `bestWave` ignores factory-fingerprint and changelog-only rows.
- **Explore-map contract**: when kickoff explores left `meta.exploreMaps` picks, 8 consecutive ships that do not touch a pick hold unlimited ULW and reprint the picks + Wave 1 reading. On-contract ships reset the streak. Stuck-wall does not increment.
...
- **Hold context**: class/contract hold merges recent mill tool-call ids into an *existing* sticky omit set (suffix only — never invents a first clip) and admits Wave 1 + picks at the tail.
