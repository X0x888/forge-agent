...
| **Same-surface hold** | 3 consecutive declared ships on the same theme (summary overlap, leftover-sibling language, **or** adjacent-share / factory schema — rotating nouns on “beside you / far stays”) → unlimited ULW holds until a different-**class** `Reading:` or `/cycle 0`. `Far stays` alone is not a schema. An explore-map pick is never mill. Stuck-wall does not increment. Capped and `/cycle 0` N+1 budgets are not held. A new noun is not a new surface. |
