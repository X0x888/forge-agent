...
The only self-stops the harness honours are a mandate fulfilled after sitting the product, the user's controls, and — with **no mandate** — the **no-progress wall** on repeating empty work: when `directExecuteStreak` consecutive synthesized cycles land no commit, or `noCommitStreak` consecutive cycle closes land no substance commit (not a dead look lease, not `FORGE_ULW_AUTO_COMMIT=0`) (`FORGE_ULW_NO_PROGRESS_CAP`, default 3). An open mandate is the job: empty cycles keep planning. A Planner `blocked` or a look that could not run synthesizes other work — it does not end the mill, and look-lease synths do not burn `synthStreak`. A substance commit resets both streaks. This is non-negotiable 2 ("never an infinite trap *without progress*") read as: do not loop the same empty class; do not abandon a mandate because a lease died or a cycle shipped nothing.
