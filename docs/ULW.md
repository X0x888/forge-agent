...
| **Consolidation cadence** | Every 4th wave is a CONSOLIDATION wave: no new scope — hostile cumulative `git diff` + the project's **full** check suite. Isolates (`node --test tests/foo.test.ts`, `python -m unittest …TestCase.test_*`) are proof=ran, not proof=✓. Skip/hang/targeted-only is proof=✗. Timeout the suite; do not skip it. |
