...
| Behavior | Detail |
|---|---|
| **Model fallback** | Off by default. After same-model 429/5xx retries exhaust, `/fallback on` (or `fallback_models` / `FORGE_FALLBACK_MODELS` / `--fallback-models`) may switch to the next same-provider model **at or above grok-4.5 high**. Weaker ids (grok-4, haiku, flash, composer, auto, …) are rejected, so unattended runs cannot silently drop intelligence. Cursor hops use wire ids (`cursor-grok-4.5-high`), never Auto. Cursor-saved ids unwrap on xAI/OpenRouter; `/provider` rebinds the chain; native providers only hop their own family. Quota/auth still switch accounts. Last hop is persisted (`lastModelFallback`) and shown on `/share` / `/status` / resume. |
...
| **Session lastError** | Provider/run failures stamp `meta.lastError` (code/message/tips); `/status`, resume, `/share`, sessions list ERR badge, HUD/tmux, `forge status --json`, and `forge run --json` fail payloads surface it; cleared on success/`/clear`/`/fork`. Doctor + `/sessions errors` tally by code (`sessionsLastErrorByCode`); `errors`/`untitled` list filters apply before the row limit |
...
| **Reasoning wall** | Default 12 min **no visible output** wall (`FORGE_PROVIDER_REASONING_WALL_MS`; `0`/`off` disables). Reasoning SSE and keepalives reset the stall timer but do not extend this. Thought-only `finish_reason=stop` (or `reasoning_wall`) is Stop, not an empty-continue — it does **not** count toward `FORGE_ULW_MAX_CONTINUES` / auto-LAST. Consecutive thought-only Stops this turn cap at `FORGE_THOUGHT_ONLY_MAX` (default 8; ends the turn only). Independent of `FORGE_PROVIDER_MAX_MS` |
| **Reasoning loop** | If hidden thought (≥3k chars) repeats the same 48-char span ≥8 times with no content/tools, cancel as `reasoning_loop`. Cuts a stuck mantra in seconds; does not fire on ordinary waves (maze working-wave max thought 2.5k, no repeated window). 12m wall remains the backstop |
...
| **Unattended cost meters** | `forge run --json` / `metrics.jsonl` / last-run session meta: `harnessUserPokes`, `admitCount`, `proofPokes`, `providerRounds`, and `guardBlocks` (Stop-guard bounces by guard: `handoff` / `proofClaim` / `report` / `todoGate` / `goal` / `ulw` / `hook` / …). Interactive REPL runs write them too (they used to be headless-only). `forge stats` prints a `harness:` row — pokes as a share of rounds, proof pokes, blocks per guard |
...
| **File mutation journal** | Successful edits append pre-images to `sessions/<id>/mutations.jsonl` (mode `0600`, ~1.5 MiB/body cap) so `/undo` / `/retry` restore **disk + chat** (OpenCode-inspired; large bodies skipped with an explicit note). Foreground `bash` / idle bang-shell / background bash workspace writes join the same journal via git porcelain delta (`FORGE_BASH_MUTATION_JOURNAL=0` off; bg snapshots at start, applies on exit or `/undo` settle). `isolation=none` spawn folds the child's journal onto the parent before cleanup deletes the child session (worktree land journals parent pre-images separately) |
...
| **metrics.jsonl** | Append-only run counters (tokens, edits, duration) under `~/.forge/metrics.jsonl` — no prompts/secrets; auto-prunes past ~2000 events / 2 MiB. Per-round `provider_round` lines live in `~/.forge/rounds.jsonl` (auto-prune 5000) so a chatty ULW run cannot evict run history. `forge prune-metrics --keep 500` cuts both |
...
| **HTTP/2 stream RST (`NGHTTP2_INTERNAL_ERROR`)** | Node `http2` `ERR_HTTP2_STREAM_ERROR` / `Stream closed with error code NGHTTP2_*` (Cursor AgentService) is a dropped connection, not generic `provider_error`. Retryable + continue-recoverable. Reconnect skips OAuth refresh (not a dead token). After same-payload retries fail, drop recovery prunes/compacts history before a new Run rebase (`conversation_history`). Cursor Connect `internal` HTTP 400 is the application-level twin (not a permanent 400 halt). `/retry` · `/compact` if it still escapes. |
| **Multi-account failover** | Same-provider accounts: proactive switch on high plan usage / cooldown / dead token; reactive switch on 429/quota; wait a short alternate cooldown (`FORGE_ACCOUNT_COOLDOWN_WAIT_MAX`, default 180s) instead of dying; team spend cap (`personal-team-blocked`) is not a missing login; session pin so concurrent TUIs do not stampede `active`; post-switch OAuth refresh; cap via `FORGE_ACCOUNT_SWITCH_MAX` (default 3); stale plan probes (>6h) ignored |
...
| **Continue-cap + ULW** | `releasedOnContinueCap` under ULW CONTINUE also flips to LAST (same helper) so length/empty caps do not leave the session stuck. Unlimited CONTINUE Stop-blocks do not trip the cap (log10). Length / empty / content_filter keep their own counter so wave 200 + one truncation is not `continue_cap`. |
...
| `FORGE_ACCOUNT_COOLDOWN_WAIT_MAX` | `180` | Seconds the quota path will wait for a same-provider alternate's cooldown before giving up (`0` = no wait, hard cap 900). A 3-minute cooldown is not a missing login |
| `FORGE_AUTH_RECOVERY_MAX` | `20` | Max mid-run OAuth refresh recoveries per agent loop |
| `FORGE_PROVIDER_DROP_RECOVERY_MAX` | `5` | Max in-loop retries after a continue-recoverable drop (`terminated`, empty stream, 5xx, HTTP/2 RST). OAuth refresh for socket/token drops; HTTP/2 / Cursor Connect `internal` reconnects without rotating creds and may compact before rebase |
