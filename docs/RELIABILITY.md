...
| **Continue-cap + ULW** | `releasedOnContinueCap` under ULW CONTINUE also flips to LAST (same helper) so length/empty caps do not leave the session stuck. Unlimited CONTINUE Stop-blocks do not trip the cap (log10). |
