...
| **Stale tool-result clearing** | Proactive microcompaction (Anthropic `clear_tool_uses` pattern): tool outputs older than the hot tail (default 10 non-system msgs; ULW floors at 10 so a parallel batch fits) and bulkier than 1200 chars are replaced by restorable stubs after the body is spooled to `~/.forge/tool-output/` (0600). Stubs say `read_file` that path — never re-run `spawn_subagent`/bash. Dumps still referenced by a session are not pruned. Runs at most every `FORGE_TOOL_CLEAR_EVERY_TURNS` (4; ULW every 2) and only when it frees ≥ `FORGE_TOOL_CLEAR_MIN_STALE_BYTES` (12k); `FORGE_TOOL_CLEAR=0` disables |
