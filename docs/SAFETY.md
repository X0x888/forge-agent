...
Forge-spawned children inherit a scrubbed env via `createChildEnv` (bash, git helpers, format-on-write, `lsp ensure`, hooks, grep, sandbox fallback). Secret-looking names and process-injection vectors (`LD_PRELOAD`, `NODE_OPTIONS`, `DYLD_INSERT_LIBRARIES`, `PYTHONSTARTUP`, `BASH_ENV`, `GIT_DIR`, `GIT_INDEX_FILE`, `GIT_WORK_TREE`, `GIT_SSH_COMMAND`, `GIT_CONFIG_*`, …) are dropped unless policy `set` reintroduces them (checkpoint temp index, hook `FORGE_*`). MCP/LSP stdio `keepSecrets` still passes `GITHUB_TOKEN` / `CONTEXT7_API_KEY` (allowlist). Other secret globs (`AWS_SECRET_ACCESS_KEY`, `DATABASE_URL`) stay stripped. **Forge/LLM provider keys** (`XAI_API_KEY`, `OPENAI_API_KEY`, `CURSOR_ACCESS_TOKEN`, `FORGE_API_KEY`, …) are always stripped from inherit. Put a key in `mcp.json` `env` to pass it on purpose. Injection is always stripped.
