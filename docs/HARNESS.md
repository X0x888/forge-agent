...
2. **`/goal` driver** (if armed and not paused, **and ULW is not armed** — an armed plan-cycle owns Stop so a goal stuck-wall cannot sit the mill down mid-mandate, and a goal block cannot swallow `Plan complete.`)
...
3. **ULW plan-cycle driver** (`src/harness/cycle/`, `docs/ULW.md`). The executor's Stop is a wave boundary: open items re-anchor; a completed or closed plan enters harness verification, fresh review, verification again, and local commit. An incomplete or failed Reviewer cannot authorize a commit. Unresolved `Must-fix` or partial/missing items enter repair; executor repairs invalidate approval and receive a fresh review, including resumed sessions saved under the old repair protocol. Verification and review repairs share `fix_rounds`. A changed gate can capture a later baseline only after a committed cycle and a confirmed clean repository root; the first baseline includes the user's existing edits. Releases include a fulfilled explicit mandate after sitting the product, user controls, or — with no mandate — the no-progress wall on repeating empty work. A Planner `blocked`, a dead look, or an open mandate synthesizes / keeps planning — it does not end the mill. Exhausted fix rounds skip the cycle and plan the next. An explicit Reviewer `blocked` closes without commit. A promptless `fulfilled` continues investigation. See `docs/ULW-AUDIT.md` for the design's remaining limits.
