...
| **Stale tool-result clearing** | Proactive microcompaction: old bulky tool outputs → restorable stubs with a `read_file` path (`FORGE_TOOL_CLEAR*`). Never “re-run” a non-idempotent tool to restore. |
